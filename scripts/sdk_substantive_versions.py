...
_NOOP_HINTS = (
    ...
    re.compile(r"本包无改动"),
    # 「无本包运行时改动」/「本包无运行时改动」—— maestro 0.97.0 / 0.93.0 / 0.92.1
    # 三条实测用的正是这个中文说法：内容确是空转（git 实证该版本 maestro 侧只动了
    # src/index.ts 里的版本常量），措辞既非规范开头、也躲开了上面全部线索正则，
    # 于是 --check 报 OK、自动清单却把三个零改动版本列进「对你有实质变更」。
    re.compile(r"无本包(运行时|代码)?改动"),
    re.compile(r"本包(运行时|代码)?无改动"),
)
