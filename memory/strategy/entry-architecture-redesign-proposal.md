...
1. **当前架构有「外人入口缺失」** —— 守密人 2026-05-06 实测验证：CLAUDE.md / BIAV-SC.md / boot-snapshot.md 全部假设读者是「内部角色」，外人接入路径不存在
2. **守密人裁定三层重构**：BIAV-SC.md 统一入口 / README.md 跳转 / CLAUDE.md 给 Light 个人维护
3. **本提议给出**：BIAV-SC.md 10 章新大纲（5 类受众分诊）+ CLAUDE.md 7 章新大纲（含 §0 警告反向跳转）+ README.md 三选项快速接入 + 13 文件改动清单 + 三批落地路径 + 6 风险登记 + 5 场景实测验收
4. **总改动量**：~13 文件 / +450 行 / 工作量约 5-6 小时（Code-site 实施）。仍在「精简优雅可维护」硬约束内——架构整合而非新增复杂度

### 决策点 1 处（请主控台 + 守密人审定）

| # | 决策 | 选项 | 艾瑞卡建议 |
|---|------|------|-----------|
| **D1** | 是否接受本提议总方向并启动批 1 | 接受 / 调整大纲 / 暂缓 | **接受 + 启动批 1** |

守密人已在 2026-05-06 裁定本质方向，剩下只是细节大纲是否照本提议走。

### 后续接力路径

详见 § 九 § 9.2 三件事——主控台决策档登记 → 起草批 1 brief → 派 Code-site 实施 + 5 场景实测。
...
| v0.1 | 2026-05-06 | § 一 ~ § 九 全部章节落档 + 摘要回填 + D1 决策点提请 | Code-strategy 艾瑞卡 |
