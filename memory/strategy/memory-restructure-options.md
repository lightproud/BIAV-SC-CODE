...
| # | 决策 | 选项 | 艾瑞卡建议 | **守密人 2026-04-26 裁定** |
|---|------|------|-----------|------------------------|
| **D1** | 总方向：增强 / 替代 / 并列 | 增强 / 替代 / 并列 | **增强** | ✅ **增强** |
| **D2** | 启动节奏：方案 1 / 2 / 3 / 暂缓 | 方案 1 / 方案 2 / 方案 3 / 暂缓 | **方案 2**（批 1 + 批 2，不动目录）| ✅ **方案 2** |
| **D3** | 主试点主题 | T1 卡牌 / T2 战略 / T3 直推 main 政策 / T4 联动 / T5 接口 | **T1 卡牌系统** | ✅ **T1 卡牌系统** |

### 3.x 守密人附加硬约束（2026-04-26）

> 「全部按你推荐来。我只有一个希望，**确保代码精简优雅可维护**。」

本约束写入主控台 dispatch brief，作为批 1 / 批 2 实施验收的**硬性条款**。具体落实：

| 落实点 | 反例（艾瑞卡禁止 Code-memory 走的路）| 正例（艾瑞卡推荐 Code-memory 走的路）|
|--------|------------------------------------|----------------------------------|
| `.searchignore` 实现 | 自创新格式 / 用 YAML / 加 GUI 配置 | 直接复用 gitignore 语法 + 单文件 ≤ 30 行 |
| `doc_class` 加权 | 引入新分类器 / 训练 ML 模型 | `memory_search.py:rerank()` 加 1 行 `score *= class_weights.get(doc_class, 1.0)`，权重表 ≤ 10 行常量 |
| 主题入口 hub | 引入新 markdown 扩展 / 多语言 frontmatter | 纯 markdown，每份 ≤ 200 行，结构化按 § 一引文 / § 二当前结论 / § 三相关档案 三段 |
| 事实卡片 schema | 自创 schema | **直接复用 `assets/data/card-system.json` v1.0 模式** + JSON Schema 校验 |
| 新增依赖 | pip 加 sklearn / spacy / 任何 ML 库 | **不引入任何新依赖**，纯 stdlib + 已有 PyYAML / jq |
| 新增脚本数 | 多文件分层 / 抽象基类 / 工厂模式 | 批 1 + 批 2 总新增脚本 ≤ 2 个 |
| 单文件长度 | > 300 行的脚本 | 单文件 ≤ 300 行；超出强制拆分必须有合理理由 |

**艾瑞卡的判断**：方案 2 之所以入选，关键正是「**改动量小 → 自然精简**」。批 1 + 批 2 全部动作的核心代码量预计 ≤ 200 行，符合守密人意图。批 3 涉及目录重构，复杂度跳一档——这也是艾瑞卡建议**先不做批 3** 的原因之一。
