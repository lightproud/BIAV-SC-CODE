...
| T41 | **SCS-REQ-REPOS-01 §1/§2/§6 定位改写与文档落簿**（与 §3 接口面同期）：POSITIONING 按「引擎层」终态重写（不认识斜杠 / 不持有钟 / 不追宿主面 / 只把一次结构化 agent 调用执行到位；含**扩展面总原则**——全部扩展面限定钩子 / 工具 / 结构化配置三种缝，不引入第四种）、**钩子契约总则写入钩子文档**（引擎拥有「何时」与契约、宿主拥有「什么」；回调生命周期不得超出所嵌 query，钩子内挂墙钟 = 走私 runner 层的钟，定位违规；既有与新增钩子一体适用）、COMPAT 降级**参照笔记**（逐条移除「待补齐」义务表述，chase 改双触发：新模型发布 / 自设新模型侧表面）、decisions.md 两条覆盖注（drop-in 兼容承诺退役→「模型面贴近、宿主面自主」；「机制归引擎、钟与装配归宿主」终裁） | 预算 | `Public-Info-Pool/Resource/repo-engineering/scs-req-repositioning-loop-support-20260717.md` §1/§2/§6（守密人 2026-07-17 已裁定，待实现；同日修订版覆盖） | 开 |
| T42 | SCS-REQ-REPOS-01 **R1 续轮注入 API**：向已存在会话追加新用户轮并继续执行返回完整结果；与 sessions/resume 统一不另起抽象；注入前可读上下文占用与累计成本；支持结构化前置内容（供 R4 ledger 注入） | 预算 | 同上需求书 §3 R1（§3 内按 R1→R5 排序） | 开 |
| T43 | SCS-REQ-REPOS-01 **R2 预算事件流**：maxBudgetUsd 之上补 `budget:threshold`（可配置比例）/ `budget:exhausted` 两级结构化事件；到帽当前轮完成后停止并产出收尾报告（累计成本 / 轮次数 / 最后状态摘要）；走既有 hook/事件机制，宿主免轮询 | 预算 | 同上需求书 §3 R2 | 开 |
| T44 | SCS-REQ-REPOS-01 **R3 压缩钩子（保留区）**：自动压缩时宿主可声明不可丢弃的结构化上下文区，原样活过压缩；有大小上限、超限报错不静默截断 | 预算 | 同上需求书 §3 R3 | 开 |
| T45 | SCS-REQ-REPOS-01 **R4 ledger 原语**：去重账本纯逻辑（记已报事件 / 查重 / 按容量时限淘汰）；可序列化（宿主自选存储，引擎不持久化）；与 R1 前置内容和 R3 保留区一行适配 | 预算 | 同上需求书 §3 R4 | 开 |
| T46 | SCS-REQ-REPOS-01 **R5 LoopControl 工具定义**（模型侧表面）：`{ action: "propose_stop", reason }` 提议语义——模型只能提议终止、结构化事件抛宿主、引擎自身行为不变；可选导出由宿主装配时注册；**唯一 chase 前置**：对照官方当前命令工具面 schema 贴近训练分布，语料不可得则自研先行并在 COMPAT 参照笔记标「待对齐」 | 预算 | 同上需求书 §3 R5 | 开 |
| T47 | SCS-REQ-REPOS-01 **R6 引擎版本声明面**：运行时可查询引擎版本 + 工具面清单及各自版本，供热更新能力层（技能 md / MCP 清单）加载时兼容校验；配套技能编写标准增补 frontmatter 引擎兼容声明字段规范条目 | 预算 | 同上需求书 §3 R6 | 开 |
| T48 | SCS-REQ-REPOS-01 **§4 斜杠退役（一刀切净）+ §7 装配验收**：同一版本内一次切净——无弃用期 / 无兼容垫片 / 无双路径（过渡态双真相源是混淆源）；引擎不再识别 / 展开斜杠，prompt 原样进装配；既有 /loop /goal 文本解析器与控制器（parseLoopCommand / createPromptLoop / goal 文本拼写接线）**整体删除**（代码 + 公开导出 + 行为测试同提交移除，输入解析归 BPT 客户端层自行实现）；**/goal 机制留引擎、入口切结构化**：保留 Stop 拦截 / 裁判反馈注回 / impossible 逃生口 / maxTurns 预算帽（均在单次 query 生命周期内），goal 改为 query options 结构化配置（目标描述 + 宿主注入评估器回调——纯函数优先或宿主自发裁判模型调用，引擎不硬编码模型选择 + 逃生策略），结构化配置为唯一入口；回归测试锁终态：`/loop ...` `/goal ...` 纯文本透传 + 源码无斜杠识别残留（搜索断言防回潮）；核心验收 = 仅用 R1–R5 公开接口拼出示例装配测试（续轮循环 / 跨轮去重活过强制压缩 / 到帽停机出收尾报告 / 提议事件到宿主循环不受影响 / **goal 结构化装配：裁判判未达标注回重驱 + impossible 逃生生效**，fake timer 零真实钟） | 预算 | 同上需求书 §4/§7（2026-07-17 修订版：§4 由「降级保留」改「整体删除」）；非目标清单见 §5（loop 编排本体归黑池 runner 层、衔接 T13 唤醒缝届时另交） | 开 |
## 已清（销案引）
