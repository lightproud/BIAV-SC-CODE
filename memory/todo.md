...
| T40 | **silver-core-sdk 缺陷扫荡暂缓的 5 个设计权衡题**（2026-07-16 五轮审计、守密人「合并」收口后挂账）：均为**真实缺陷但无干净安全修法**、须守密人裁定修法方向者，艾瑞卡未擅动——① **anthropic 空消息**（`transport/anthropic.ts`：未签名 thinking 剥离可把 assistant 消息清空、破坏 role 交替，须角色交替感知修法）；② **重复 sidechain_start**（`subagents/runtime.ts`：每次 SendMessage 续跑重写 sidechain_start 标记，转录出现多对 start/end，须标记生命周期重构）；③ **MCP 工具 content specifier 失配**（`permissions/rules.ts`：非 PRIMARY_ARG_FIELD 表内工具的 scoped specifier 比对 JSON blob 永不匹配，唯一修法=对序列化 JSON 模糊子串匹配、自带误配风险）；④ **openai tool_call id/index 键分裂**（`transport/openai.ts`：同一 call 的 fragment 分别带 id 与 index 时分裂成两 buffer、末尾多出空名块，热路径罕见，须 id↔key 别名协调）；⑤ **会话末轮 + 迟到后台子代理用量漏计**（`query.ts`：memory 会话末轮 driveTurn 的真实花费、与末结果 yield 后才完成的后台子代理用量，均不落任何 result/`mgr.usage()`，须发合成结果或状态消息补账）。逐个可点名让艾瑞卡定向处置。 | 裁定 | 本会话 SDK 五轮扫荡（PR [#699](https://github.com/lightproud/brain-in-a-vat/pull/699) 合并 `e87dde9d`）+ `projects/silver-core-sdk/CHANGELOG.md` 0.62.2/0.62.3 deferred 注 | 开 |
