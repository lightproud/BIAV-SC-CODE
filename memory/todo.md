...
| T36 | **社区数据采集必要性重估**：wiki 使命取消后，社区数据采集的必要性进入重估——**采集不停**（现状照跑），重估结论另行裁定（保留 / 收缩源清单 / 降频 / 退役均属可能口径，与使命#1 黑池信息入口的消费需求对照评估） | 裁定 | 守密人 2026-07-12「wiki 使命取消 + 银芯定位收敛」裁定条① | 开 |
| T37 | **news 下半场推送形态待构想**：归档已稳，定期以何种形态推送到团队（黑池侧消费面）待构想——稳态期唯一开放议题；与 T34（推送通道，仅恢复定时报告时激活）相邻不重合：T37 议「什么形态的内容产品」，T34 议「走哪条通道送达」 | 裁定 | 守密人 2026-07-12「稳态维护期」裁定条③ | 开 |
