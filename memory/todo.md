...
| T19 | CI 硬门禁 Ruleset 勾选：只勾 required `test`（不勾 up-to-date） | 守密人 2026-07-11 在 GitHub Settings → Rules 手动建 Ruleset 生效（required check `test` + Restrict deletions + Block force pushes,github-actions 入 bypass 护每小时机器直推）；首个受门禁 PR = 本销案 PR;裁定源 `decisions.md` 2026-07-11 记忆系统条⑤ |
