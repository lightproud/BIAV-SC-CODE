...
| T41 | **SCS-REQ-REPOS-01 §1/§2/§6 定位改写与文档落簿**（与 §3 接口面同期）：POSITIONING 按「引擎层」终态重写（不认识斜杠 / 不持有钟 / 不追宿主面 / 只把一次结构化 agent 调用执行到位）、COMPAT 降级**参照笔记**（逐条移除「待补齐」义务表述，chase 改双触发：新模型发布 / 自设新模型侧表面）、decisions.md 两条覆盖注（drop-in 兼容承诺退役→「模型面贴近、宿主面自主」；「机制归引擎、钟与装配归宿主」终裁） | 预算 | `Public-Info-Pool/Resource/repo-engineering/scs-req-repositioning-loop-support-20260717.md` §1/§2/§6（守密人 2026-07-17 已裁定，待实现） | 开 |
...
| T48 | SCS-REQ-REPOS-01 **§4 斜杠退役 + §7 装配验收**：引擎不再识别 / 展开斜杠命令，prompt 原样进装配（纯文本 `/loop ...` = 透传，回归测试锁定）；既有 /loop /goal 解析器**不删除**、降级可选导出工具包（测试全保留）；/goal 内建钟评估（钟在单次 query 生命周期内则留，否则降工具包，结论记 decisions.md）；核心验收 = 仅用 R1–R5 公开接口拼出示例装配测试（续轮循环 / 跨轮去重活过强制压缩 / 到帽停机出收尾报告 / 提议事件到宿主，fake timer 零真实钟） | 预算 | 同上需求书 §4/§7；非目标清单见 §5（loop 编排本体归黑池 runner 层、衔接 T13 唤醒缝届时另交） | 开 |
## 已清（销案引）
