...
## 九、附录 — 批 2 H5 字面实施 + H8 candidate 实验（同日续测）

> 守密人 2026-05-03 选 A：直接授权艾瑞卡在本会话内开干批 2 子集。下方记录两次新测量。

### 9.1 H5 字面实施（dict 结构升级）

**改动**：`scripts/memory_search.py:doc_class_weight()` 由单 `if` 链升级为 `DOC_CLASS_WEIGHTS` dict 表。当前唯一 marker：

```python
DOC_CLASS_WEIGHTS = {
    "/session-digests/": 0.4,   # verbatim conversation logs
}
```

**实质**：与 4-26 commit `4d63b8ca` 的 land 状态**等价**。本次新增唯一价值：未来扩展无需改函数体。

### 9.2 H8 candidate 实验（艾瑞卡建议 §八#3，已回滚）

**实验设置**：在 `DOC_CLASS_WEIGHTS` 加入 `"dispatch-brief-": 0.5` 与 `"/dreams/": 0.6`。

**实测 M3 对比**：

| 状态 | M3 非 SD 占比 | Delta |
|---|---|---|
| batch1 post（仅 .searchignore + 5 hubs）| 52.0% | 基线 |
| batch2 H5 strict（仅 session-digest 0.4 dict）| 44.0% | -8 pp |
| batch2 H5+H8 candidate（含 dispatch-brief / dreams 降权）| 44.0% | -8 pp |

**结果**：H5+H8 与 H5 strict 的 M3 一致 = 44%，**两者均比 batch1 post 退步 8 pp**。

### 9.3 退步根因诊断

逐 query 比对 batch1 post → H5 strict 发现，4 个 query 的 top1 退步：

| # | batch1 post top1 | H5 strict top1 | 退步原因 |
|---|---|---|---|
| Q4 | `p/wiki/.../game_config_at.txt` | `m/session-digests/20260503-140040-753630f7.md` | **新生 session-digest 抢位** |
| Q8 | `m/active/contribution-protocol.md` | `m/decisions.md` | active hub 排名波动（同质权威档案）|
| Q15 | `m/silver-memory-enhancement-plan.md` | `p/wiki/.../game_config_at.txt` | 同质权威档案（弱退）|
| Q20 | `m/active/silver-blackpool-interface.md` | `m/session-digests/20260503-140040-753630f7.md` | **新生 session-digest 抢位** |

**结构性根因（meta-test 污染）**：本会话过程中 SessionEnd hook 落档新 session-digest（`20260503-140040-753630f7.md`）含 batch1 post-baseline 测试时的 20 query 字面文本——sem 极高。Q4 / Q20 这两个 query 直接命中本次会话产生的 digest，**无论 H5 / H8 系数怎么调都压不住**（自我引用闭环）。

### 9.4 H8 candidate 不采纳

H8（dispatch-brief / dreams 降权）实测有得有失：

| 收益（H5+H8 vs H5 strict）| 损失 |
|---|---|
| Q2 命轮旋律 → game_config_at（lua 真数据）| Q4 退步至新 session-digest |
| Q3 Aequor → steam_review（玩家发言）| Q15 退步至 game_config_at |
| Q5 卡牌三类 → game_kv（lua 真数据）| Q8 退步至 decisions（同质）|
| Q12 RAG 链 → rag-performance-baseline | — |

**M3 净效果**：0（H5+H8 vs H5 strict 同为 44%）。**得失抵消，不采纳**。

### 9.5 M3 60% 门槛不可达诊断

H5 字面授权（rerank 加 1 行 + ≤10 行常量表）下，M3 60% 在当前数据集**结构不可达**。原因：

1. **hot session-digest 自污染** — 每次 baseline 测量都产生新 session-digest，sem 高于绝大部分权威档案
2. **降权效应有上限** — session-digest 0.4 已是较激进系数，再降至 0.2-0.3 风险过大（破坏 Phase 2 M1-M4 时间表 / distill hook 设计取舍 这类 query 的合理 session-digest 命中）
3. **.searchignore 30 天阈值未触发** — 252 份 session-digest 全部 < 21 天

### 9.6 突破 60% 的 3 个候选路径（需主控台审定，超出 A 授权）

| 路径 | 工作量 | 副作用风险 |
|---|---|---|
| `.searchignore` 阈值 30 → 7 天（仍是 H4 字面 gitignore 语法）| 极小 | 中——切断 7 天内的 session-digest 自查询能力 |
| 加「hot session-digest 隔离」逻辑：< 24h 不入索引 | 小 | 小——本会话查不到本会话的产物（合理）|
| H7 T1 卡牌事实卡片化推进，让游戏类 query 不依赖 session-digest | 大 | 极小——是批 2 主任务 |
...
| v1.1 | 2026-05-03 | 附录 §九：H5 字面实施（dict 化）+ H8 candidate 实验（不采纳）+ M3 60% 不可达诊断 | Code-memory 艾瑞卡 |
