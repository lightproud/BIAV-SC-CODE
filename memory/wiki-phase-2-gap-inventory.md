...
## 2026-07-02 补充：命轮→角色归属为一手数据盲区（P2-7 调查结论）

体质改进批次曾计划把命轮（启灵）效果接入角色详情页，调查后**判定不可按一手数据实施**：

- `AwakerPotency.lua`（1,035 条）仅含 `PotencyName` / `PotencyDesc`，**无 awaker 归属字段**；
- `AwakerConfig.lua` 无任何 Potency 引用键；
- 效果描述正文虽偶提角色名（如「获得图鲁攻击力的…」），但按行文猜归属违反
  「一手解包为唯一源、禁合成」纪律（2026-06-15 裁定），不做。

**结论**：命轮↔角色映射是新的已知解包缺口。补全路径：重跑 Frida hook 时留意
Awaker 侧引用 potency id 的配置表（可能在未捕获的 chunk 中），或等社区源可交叉
验证时按 `translation_source: "community"` 同款置信标注引入。上表「命轮 effect —
AwakerPotency.lua（本地已有）」指效果**文本**已有，**归属**仍缺，特此订正口径。
