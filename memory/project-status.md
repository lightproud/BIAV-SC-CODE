...
- **#4 质性能力 probe 已落地**（守密人「针对专有能力 grep 还是好用」逼出）：真相=hit@k 是 grep 主场（只测找文本），KB 真价值在检索之后的结构化知识（层/身份/边界），grep 结构上给不了、hit@k 测不出。`scripts/kb_qual.py` 三 probe：层判定（16 平台 KB 区分 16/grep 0，防 lesson #30）、身份消歧（唯一 type=character 规范，KB 5/grep 0）、边界枚举（KB 可枚举 72 角色/59 全量、grep 给不了）。**实测 KB 3/3、grep 0/3**。守护 `tests/test_kb_qual.py`。
- **评判体系四仪器齐**：#1 黄金集（检索 hit@k）/ #2 使用遥测（追踪）/ #3 反事实 A/B（对照 grep）/ #4 质性 probe（层/身份/边界，测 hit@k 测不出的 KB 真价值）。
