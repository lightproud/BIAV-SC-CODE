...
| **bpt-agent-sdk**（Claude Agent SDK 公开信息再现 · 银芯→黑池单向输出物） | **v0.11.0 已发货（2026-07-06 评估实测）**：TypeScript 重实现（公开信息再现、自研引擎），直驱 Anthropic Messages API（fetch+SSE，无 CLI 子进程），**1414 单测全绿 + 2 skipped（真 bwrap 隔离）、tsc/build exit 0**（实机取证，非文档自陈）；对官方 SDK **0.3.201**（已追平漂移）约 90%+ 表面等价，COMPAT 台账 FULL 55 / PARTIAL 39 / ACCEPTED 10 / UNSUPPORTED 7；一致性金字塔 L1–L5 全封顶、首轮真 API L5 两臂打平 88.9%。中间里程碑（v0.4→v0.11：观测流 / 提示词 v5 / 生成器·分类器 / 沙箱 / E1–E5 引擎对齐 / 接口逐条对账）详见下方专节 | 艾瑞卡会话 | 无阻塞待办；投资路线（2026-07-06 评估 backlog，待逐项裁定优先级）：**P2** 收敛 39 项 PARTIAL（drop-in 保真主战场）/ **P3** 漂移哨兵「只报」升「报+自动开对齐 PR 草稿」/ **P4** 补「新维护者 30 分钟上手」降总线因子；评估全文 `Public-Info-Pool/Resource/repo-engineering/bpt-agent-sdk-evaluation-20260706.md`；**动手前必读** `projects/bpt-agent-sdk/CONTEXT.md`，定位见 `projects/bpt-agent-sdk/docs/POSITIONING.md` |
