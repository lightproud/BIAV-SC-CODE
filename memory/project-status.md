...
- **一致性测试套件（2026-07-05 拷问定稿开工，设计蓝图
  ...
  **799 单测全绿（29 文件）**；一致性验证体系 M1-M4 全部封顶，唯余首轮真 L5 dispatch 出正式对照报告
...
- **待办**：② **Phase 2 语义铁证**（**设计注**：证「向量独胜语义近邻」不是 `kb_ab` 四臂扩展——kb_ab 测的是 OKF **概念**检索，向量腿检索的是长尾**消息**，目标空间不同；需**新建 paraphrase-recall harness**：query=某消息的换说法改写、零共享 token，验向量能召回该消息而 grep/脊柱不能。宜在 CI 对真索引跑）；真索引落**持久处**（CI 传 Release community-assets，运行时 `restore_release_data` 还原，令艾瑞卡可实际用 `kb_vector_search`）；③ **Phase 3 厚锚**（别名 AI 自动识别 + 先锚后扩，独立子工程）。设计全文 `Public-Info-Pool/Resource/proposal/silver-core-vector-leg-design-20260705.md`，决策见 `memory/decisions.md`。
