...
- **✅ Phase 2 语义铁证 harness 已落（2026-07-05）**：`scripts/kb_semantic_ab.py`（paraphrase-recall 四臂 vector/grep/grep_strong/spine，主分 `vector_exclusive_win_rate`；自足黄金现场嵌入、不依赖已建索引）+ `tests/kb_semantic_golden.jsonl`（7 条种子，query=真社区消息零共享-token 语义改写，出身牌+防火墙齐）+ `tests/test_kb_semantic_ab.py`（诚实性不变量=grep/脊柱恒 0 + stub 负控 + 确定性，7 测零网络）+ `.github/workflows/kb-semantic-proof.yml`（CI 真 Voyage 门，stub/voyage 并列出 delta 铁证）。**stub 实测**：grep/grep_strong/spine 全 0（黄金真不可达）、stub vector 0.29（哈希碰撞噪声，证词法袋赢不了语义）。**真胜负数字待 CI dispatch `kb-semantic-proof.yml`**（需 Voyage，本会话取不到 key）。
- **待办**：② **chunk2 完成**——CI 建真索引传 Release + 运行时 restore + 查询嵌入（(a)(b) 裁定已解锁，落地待推）；③ **Phase 3 厚锚**（别名 AI 自动识别 + 先锚后扩；(c) 裁定已解锁，下游于向量腿降级契约=已落）。设计全文 `Public-Info-Pool/Resource/proposal/silver-core-vector-leg-design-20260705.md`，决策见 `memory/decisions.md`。
