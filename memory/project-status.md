...
| 事实 | 值 | 权威源 |
|------|----|--------|
| Silver Core Agent SDK 版本 | `1.8.3` | `projects/silver-core-sdk/package.json` |
| Silver Core Maestro SDK 版本 | `1.8.3` | `projects/silver-core-maestro-sdk/package.json`（与 agent 锁步同号）|
| testbed 试金石 | `0.0.0`（private，永不发布）| `projects/silver-core-testbed/package.json` |
| agent SDK 源文件 / 测试档 | 140 / 226 | 磁盘实况 |
...
- **v1.8.3（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.3 为审计第二十四波（折叠比例旋钮缺 NaN 守卫致自动折叠整场静默失效 · `ReportLedger.record` 不校验字段类型致折叠中途抛裸 TypeError），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.8.2（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.2 为审计第二十三波（校验器行式摘要未折行 · 两份会话存储撕裂尾自愈被失败写入解除 · `loadInfo` 缺 uuid 去重），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.8.1（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.1 为审计第二十二波（命令前缀分类器不回校命令 · 钩子条件判定器不围栏不可信输入），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.8.0（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.8.0 新开后台任务第二投递通道（`Options.onBackgroundEvent`）并借此让 `abortAll()` 真的发终止事件，家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.7.0（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.7.0 关掉两项守密人裁定（OpenAI 空名工具块 · Bash 输出上限改真总量），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.6.0（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.6.0 为审计第二十一波（结构化输出严格提取 / 公开 token 估算器 / 文件回滚计划三处静默失败），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
- **v1.5.0（2026-07-29）**：**锁步对齐**，无本包运行时改动。agent SDK 1.5.0 为审计第十九、二十波（权限规则、会话目录授权、钩子聚合、MCP 配置加载器等十余处真实缺陷），家族版本钟锁步（守密人 2026-07-18 裁定）故本包同步升位。
