...
- **待办**：① 守密人配 CI secret `VOYAGE_API_KEY`；② **Phase 2**（`build-community-vectors.yml` CI + `kb_ab` 扩四臂 + `kb_golden_gen` 加「语义近无边」题类，证「语义独胜」铁证——需真 Voyage 索引）；③ **Phase 3 厚锚**（别名 AI 自动识别 + 先锚后扩，独立子工程）。设计全文 `Public-Info-Pool/Resource/proposal/silver-core-vector-leg-design-20260705.md`，决策见 `memory/decisions.md`。
