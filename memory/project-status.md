...
- **待守密人裁两决策（「运行时可用」完成前置，均非纯代码）**：**(a) 索引落存路线**——bounded 索引 git-commit（简单、但周期重建的 MB 级 blob 累积进 git 史，与本仓瘦身冲突）vs **Release community-assets + restore**（合本仓「二进制→Release、git 只留指针」范式，艾瑞卡倾向此）；**(b) 运行时激活**——需守密人在会话/远端环境配 `VOYAGE_API_KEY` **且**装 `voyageai` 包（实测二者皆缺，缺任一 `kb_vector_search` 永久降级回 kb_search）。
- **待办**：② **Phase 2 语义铁证**（新建 paraphrase-recall harness——非 `kb_ab` 四臂扩展，kb_ab 测概念、向量腿测消息，目标空间不同；自足黄金集现场嵌入、stub 负控对比 chance 地板、真胜负 CI Voyage 断言）；③ **Phase 3 厚锚**（别名 AI 自动识别 + 先锚后扩，独立子工程；下游于本块降级契约，且需先裁 mention 边范围 + 别名 A/B 改立关系腿两点）。设计全文 `Public-Info-Pool/Resource/proposal/silver-core-vector-leg-design-20260705.md`，决策见 `memory/decisions.md`。
