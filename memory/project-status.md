...
- **#1 黄金问题集已落地**：`scripts/kb_eval.py`（评分器）+ `tests/kb_golden_questions.json`（18 题真实守密人风格问题→该 surface 的概念 id 子串）+ `tests/test_kb_golden.py`（回归守卫）。度量**需求侧有效性**（人要的检索得到吗，hit@k + MRR），区别于覆盖哨兵的供给侧完备。诚实基线 **hit@3=0.78（14/18）、MRR 0.75**，门槛 0.68 守回归。记分卡 `python3 scripts/kb_eval.py`。缺口（改进目标）：采访/卡牌/banner 等**英文标题概念被中文查询漏掉**——留待后续「优化检索」增量（先度量后优化）。
...
- **评判体系三件套齐**：#1 黄金集（打分）/ #2 使用遥测（追踪）/ #3 反事实 A/B（对照）。
