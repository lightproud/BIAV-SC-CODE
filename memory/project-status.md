...
> + 别名 A/B 关系腿，MCP 工具 10→11，详见「向量检索腿」节）。
...
- **待办**：合并后 dispatch `build-community-vectors.yml`（真索引传 Release）+ `kb-semantic-proof.yml`（真 Voyage 语义铁证数字）；`extract_aliases.py` 生成期批量抽取跑第一轮（manual-seed 之外喂大侧表）。设计全文 `Public-Info-Pool/Resource/proposal/silver-core-vector-leg-design-20260705.md`，交接档 `Public-Info-Pool/Resource/repo-engineering/kb-vector-remaining-handoff-20260705.md`，决策见 `memory/decisions.md`。
