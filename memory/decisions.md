...
| 架构差距分析 + 8 项改进批量实施（JSON Schema、冲烟测试、Dependabot 等） | 全局 |
...
### 2026-06-21 采集编排器整体规划：三层定性 + T2 归档器独立边界（守密人选 A）

**背景**：守密人要求退一步「将整个采集编排器整体规划」，而非继续逐采集器零敲。盘点
8 条采集工作流 + 18 个脚本后定性：采集层并非一锅，而是**职能三层、产出三个不同目标**。

| 层 | 职能 | 脚本 | 产出 | 节奏 |
|----|------|------|------|------|
| **T1 新闻流采集** | 抓近期条目入流 | `aggregator.py`（唯一入口）→ AC 平台 + 内部调 `collect_global` 全球平台 | `news.json` / `news-raw.json` | 每时 :00（`update-news.yml`）|
| **T2 数据层归档/采集** | 全量落档，供 T1 与黑池消费 | `discord_archiver`(→`data/discord/`)、`collect_fanart`(→`data/fanart/`)、`collect_video_comments`(评论档)、`archive_platforms`(新闻按日/平台落档)、`archive_discord`(超60天冷归档→Releases) | `data/*/` 各档 | 各自独立错峰 |
| **T3 维护/回填** | 补缺/补媒体/修断档 | `repair_gaps` `backfill_*` `download_media` `backfill_forum_starters` `discord_list_guilds` | 修正既有档 | 随附 / 手动 |

**核心裁定**：守密人 goal「合并所有采集器到 AC」**精确落在 T1**，且 T1 已于
ARCH-01（上条）合并完成。**T2 归档器保持独立轨道，绝不并入 AC**，理由三条——
(1) 产出目标不同（T2 写 `data/*` 全量档 vs T1 写 `news.json` 流快照），并入即职责污染；
(2) 节奏有意错峰（discord_archiver :15/:30/:45/18:00 四轨、fanart/comments 每日 02:00），
塞进每时 :00 会砸掉错峰与历史偿还；(3) 存在上游→下游依赖（`discord_archiver` 落归档 →
T1 `fetch_discord_local` 读归档入流），并入即「把水源搬进水龙头」逻辑倒置。

**今日核验两处疑似不一致，均证伪（§4.2 R3 据实报告）**：
- **youtube「残留」非事故**：ARCH-01 明文「fetch_youtube 与其单测保留在 aggregator_collectors，
  仅退出生产编排」。双采已根除（AC line 173 路由 youtube→global、不进生产 fetcher 链），
  保留的 327 行 + 单测系有意为之，删之反违裁定。`test_aggregator_collectors` 103 passed 佐证。
- **discord 三脚本非冗余**：`discord_archiver`(采集/1175行) / `backfill_forum_starters`(补帖首缺口) /
  `archive_discord`(超60天冷归档轮转/245行) 零功能重叠，是「采集→补缺→冷归档」清晰流水线。

**T2 归档总入口评估**：暂不建。错峰 cron 系规避各服务器 API 限流的刻意设计，强行单一入口反害。
T2 各归档器维持独立 workflow。**结论**：采集编排器已是「T1 单入口 + T2 独立归档 + T3 维护」
清爽三层，无破损待修；本规划仅为既成清爽态的定性固化与边界封档。
...
| 2026-03-29 | 架构差距分析 + 8 项改进批量实施 | 对标业界最优实践，补齐数据验证(JSON Schema)、冲烟测试、Dependabot、共享CSS变量、404页面、爬虫降级保护、memory时间戳 | 全局 |
...
| 2026-04-19 | BPT 整条战线直接删除（bpt-web / bpt-desktop / bpt-next / graphify-ext / occ-local） | 守密人明示"BPT 不在银芯中开发"，删除比迁出更干净，不背技术债 | 全局 / BPT |
