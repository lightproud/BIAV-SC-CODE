...
| 2026-06-15 | SessionStart 同步 hook（`session-start-sync.sh`）从 `settings.json` 退役 | 守密人裁定该钩子「有点烦」（每次开工强制重写 local main 指针 + 备份 ref 只增不删 + 固定 fetch 拖慢开工），从 `settings.json` 移除注册，脚本文件保留备查可随时重新挂回。代价：失去 lesson #28 的 HTTP 413 推送堵塞自动防护，需人工留意 local main 分叉 | 全局 / 基础设施 |
