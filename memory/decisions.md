...
| **入口架构重设计 — 守密人 2026-05-06 裁定**：触发于守密人 5-6 实测「外人接入路径不存在」（外人被引导到主控台交接手册 → 错位）。Code-strategy 5-6 落档 `memory/strategy/entry-architecture-redesign-proposal.md` v0.1（10 章 5 类受众分诊大纲 + 13 文件改动清单 + 3 批落地 + 6 风险登记 + 5 场景实测）。**守密人三条核心规则**：(1) **统一入口 = `BIAV-SC.md`**（含外人 + 内部所有角色，10 章新结构按 §1 受众分诊：§1.1 Light → CLAUDE.md / §1.2 Studio 团队 → §3+§4 / §1.3 社区贡献者 → §3+§5 / §1.4 外部观察者 → §3+§6 / §1.5 银芯内部 AI → §3+§7+§8）；(2) **`README.md` = 项目门面 + 跳转入口**（顶部加 §1 快速接入约 25 行：一句魔法 / git clone / 仅浏览 三选项）；(3) **`CLAUDE.md` = Light 个人维护备忘**（不再是 AI 入口，§0 加警告章让 Claude Code 平台启动时自动跳转 BIAV-SC.md）。**守密人硬约束继承 4-26**：「精简优雅可维护」——单文件 ≤ 350 行 / 不引入新依赖 / 三文件总行数变化 ≤ +500 行 / 章节大纲遵循提议 § 四 § 五。**3 批落地**：批 1 重写 BIAV-SC.md + CLAUDE.md（不动 README）→ 批 2 改 README + 间接引用更新（dispatch-brief / methodology / CONTEXT.md）→ 批 3 守密人对 Claude.ai 一句魔法实测。**5 场景实测验收**（S1 Studio 同事 / S2 社区贡献者 / S3 外部观察者 / S4 银芯内部 Code-* / S5 主控台接班）。**实施派 Code-site**（接口规范文档 + 主站对外门户专责）。**回滚**：直推 main 政策下 1 次 git revert 即可。详见 `memory/strategy/entry-architecture-redesign-proposal.md` + `memory/dispatch-brief-code-site-entry-redesign-batch1.md` | 全局 / 入口架构 |
