...
| **仓库瘦身方案 — 守密人 2026-06-20 裁定（/grill 会话 B 分支）**：体量诊断 discord 3.1G + fanart 588M 占工作树 96%，根因是决策 179/199 立了 Releases 归档却从未写脚本。裁定**不改写 git 历史**（避撞 main Ruleset + 破坏现有 clone/采集基线），走 Releases 月归档路线：(1) **Discord** 落实既有决策 179——git 留 60 天 + 月归档 Releases + 删 git，日统计永久留；(2) **fanart 月归档 Releases**（新增，同 discord 模式，git 留近期 ~30 天 + 留 manifest 索引）；(3) **vectors.json.gz 8.7M 孤儿本会话已清**（语义检索 6-20 退役遗漏的残留，无脚本引用）+ 修 .gitignore 陈旧注释。§4.1 全量档案纪律由 Releases 接管保全。方案 + 归档脚本设计 + 首次归档 runbook 见 `memory/strategy/repo-slimming-plan.md`。**待实现**：`projects/news/scripts/archive_to_releases.py`（幂等 / dry-run / 上传校验通过才删）+ workflow（受 lesson #38 限制需守密人本地加）+ 首次不可逆大归档（守密人盯执行）。`.git` 1.1G 历史体积留待真瓶颈再评估（延续决策 178 精神）| 全局 / 仓库瘦身 |
