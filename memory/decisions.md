...
| **CI 硬门禁重启 — 守密人 2026-07-02 裁定（体质改进批次提案 §三点五采纳）**：main Ruleset 重新启用 required status check `test` 并勾选 Require branches to be up to date before merging。前提已备：test/mutation-test 工作流 sparse checkout（排除 2.4G 档案层，工作树约 100MB、全量单测约 5 秒）+ PR 触发无 paths filter（检查必报告，不再卡 Expected）。**覆盖 2026-06-21「必需 CI test 检查已撤 / 自查自合」裁定**：合并前仍建议本地跑全量 pytest，但机器门禁恢复为硬约束；up-to-date 强制项系 #373/#374「两 PR 各自绿、合起来红」对冲的真解药。会话侧合并流程相应变化：PR 须等 test 通过且基底最新方可合并（可用 auto-merge）。CLAUDE.md §7.6 同步 | 全局 / 工程纪律 | **覆盖 2026-06-21「自查自合——必需 CI test 检查已撤」裁定**（该条移交归档层） |
