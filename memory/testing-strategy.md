...
**archive_layout + news_common（2026-07-11 守密人裁定扩员）**：孪生档
`tests/test_mut_archive_layout.py` + `tests/test_mut_news_common.py`。news_common 首跑
178 存活，三轮补断言杀至 30（累计击杀 148）：一轮杀签名默认值 / 实参常量 / 缓存语义 /
序列化原样文本（178→114），二轮冻结时钟精确钉 MM-DD/MM/DD/HH:MM 分支——短横斜线日期
形态与 seconds 单位此前完全漏测（114→45），终轮杀保留段 IP / http 元组 / 308 跳转码 /
1800s 缓存边界 / 两级 mkdir / fdopen 编码实参（45→31，含 archive_layout 1）。
archive_layout 首跑揪出一个**测试选源错误级盲点**：steam 自身是折叠源，原「normal
source 跳认领子目录」测试从未走到认领跳过分支——唯 taptap（非折叠宿主）真正执行该
路径，纠正后 `CLAIMED_SUBTYPES.get(source)` 变异应声而死。另暴露并修复 dated_files
「同目录夹具下路径序碰巧等于日期序」的假绿（跨目录夹具后排序键变异翻车）。

### 已登记的等价变异体（survivor 白名单）

以下存活体经人工判定为**等价变异体**（改了源码但行为无可观测差异，不可杀），triage 视为可接受。
当前 gate 共 46 个存活（旧四靶 15 + 2026-07-11 扩员两靶 31），全部落在此表内：
...
| archive_layout | `iter_source_files` 认领跳过 `continue` → `break`（×1） | 仅在 rglob 恰先遍历到认领目录时可观测——pathlib 不保证目录序，确定性击杀需假设遍历顺序（脆测比在册存活更糟） |
| news_common | `_resolve_safe_ip` 拒绝链布尔重排（×2） | py3.11 `is_private` 已覆盖 loopback / link-local / 240/4 保留段，防御纵深子句被上游短路、不可观测（守卫行为不变） |
| news_common | `_PinnedIPAdapter.__init__` 丢 `*args` / `**kwargs`（×2） | 唯一调用点不传额外实参，空展开等价 |
| news_common | 报错文案 `[:80]` → `[:81]`（×4） | 纯观感截断宽度，无行为语义 |
| news_common | `parse_relative_time` 空白入参路径（×1） | 空白串两条路径殊途同归返回同一 `(now, True)` |
| news_common | `replace("Z", ...)` 的 Z/z 变体（×2） | py3.11 `fromisoformat` 原生接受 Z/z，replace 在本解释器上冗余 |
| news_common | 英文相对时间正则大小写翻转（×1） | `re.IGNORECASE` 下大小写无差 |
| news_common | `delta_map.get(unit, timedelta())` 默认值（×2） | 正则捕获单位集合 ≡ 字典键集合，默认值不可达 |
| news_common | `get_with_retry` 的 `last_exc` 记账（×2） | 供给不可达的防御性 `raise last_exc`（循环末次必 raise） |
| news_common | spi / wbi 的 `.get(..., {})` 默认值与缓存键下标（×8） | 变异触发的异常落在宽 `except` 内，回退返回值与原路径相同（含 wbi 80/81：KeyError 被兜、`.get` 回退取到刚写入的同一值） |
| news_common | wbi 缓存检查 `ts` 默认值（×3） | `mixin_key` 在而 `ts` 缺的缓存态不存在（两键同笔写入），分支不可达 |
| news_common | `sub_key` 的 `split('.')` 变体（×2） | `.png` 尾巴落在 raw 串 64-67 位，混淆表只选 0-63 位——mixin key 逐字符不变（构造性等价） |
| news_common | `json.dump(..., ensure_ascii=None)`（×1） | None 为假值，与 False 同义，字节级同产出 |
