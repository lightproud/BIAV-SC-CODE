...
**一句话摘要**：银芯（公开层）与黑池（内部层 SVN + Qoder）通过**严格单向流动**协作——银芯采集公开信息单向输出至黑池，黑池任何形式（代码 / 数据 / 设计 / 反馈）都不允许写入银芯仓库。指导关系由 Code-BPT（技术层）+ 主控台（战略层）通过「人工对话搬运」协议执行。
...
| 系统 | 层级 | 存储 | 内容性质 | 访问范围 |
|---|---|---|---|---|
| **银芯（BIAV-SC）**| 公开层 | GitHub（lightproud/brain-in-a-vat）| 公开信息 + 方法论验证 + 社区共建底座 | 任何人可读 |
...
- **战略层指导者** = 主控台（艾瑞卡 opus4.7 长期战略锚点）— Phase 边界 / 跨子项目影响 / 决策档案
- **技术层指导者** = Code-BPT — BPT 设计 / 架构 / 方法论 deep-dive 对话
- **指导协议** = `memory/bpt-guidance-protocol.md` v0.2「人工对话搬运」— 守密人作为「学习者」从指导对话中学习概念，**不做 harness 自动化**
- **搬运包格式**（Code-BPT 产出）= 指导主题 + 背景 + 具体建议 + 引用档案 + 验收问题
...
- `memory/active/mission-v2.0-three-pillars.md` — v2.0 三新使命，使命 #1 即「黑池公开信息入口」
...
| 指导对话 | Code-BPT（技术层）+ 主控台（战略层）双指导，搬运包发守密人人工搬运 |
