...
## 追加归档（2026-07-29，为 project-status.md 行数上限腾位；原文逐字保留）

> 起因与 2026-07-28 那次同：`memory/project-status.md` 撞 520 行上限，而上限**不抬**
> （抬上限等于承认状态档可以变目录，且其调整须守密人裁定）。迁出的是最老的两条逐版
> 叙述，逐版全文本就以 `projects/silver-core-sdk/CHANGELOG.md` 为唯一权威，此处逐字留档。

- **v0.93.0 / v0.92.1 / v0.92.0（2026-07-27~28）**：recap 截断丢最新进度（BPT P1，3 小时活锁事故根因；改头尾双保留 + 截断纪律注册表扩到全 `src/`）· 自动续跑时被拒绝的控制面覆写仍被留存并重放（悬空 Promise 可终止进程）· **Workflow 改为真异步启动**（对调用方行为破坏性：立即返回 `{status:'async_launched', taskId}`，接进既有后台 id 空间，新增查询级 `ToolContext.lifeSignal`）。三条逐版全文见 `projects/silver-core-sdk/CHANGELOG.md`。
...
## 2026-08-02 二次下沉（Hermes 换轨会话，同一治理规则执行）

> 状态档 SDK 节按 2026-07-27 治理规则只留「定位 + 最近版（v2.2.3）」，以下 v2.2.2 → v0.94.0
> 逐版叙述 + 一段档头行早先已佚失的 Windows 语义轮续行（原文照迁，不改一字）下沉至此。
> 逐版全文权威仍是 `projects/silver-core-sdk/CHANGELOG.md`。
