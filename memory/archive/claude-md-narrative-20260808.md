# CLAUDE.md 叙述层归档（截至 2026-08-08）

> **⚠ 归档层，不作运行时约束**（守密人 2026-08-08「提前整理」令）：本档收 `CLAUDE.md` 在
> 2026-08-08 整理中**移出正文的历史叙述、演进历程与长解释**。移出的都是「怎么走到今天」，
> 留在正文的是「今天该怎么做」。
>
> **为什么移**：CLAUDE.md **每会话强制加载**，长度直接从每个会话的上下文预算里扣；它自己 §5
> 开篇就写着「本节是路由器，不是目录」。整理前 31,589 字符，其中硬约束区（§0 定位与身份门控 /
> §1.1 + §1.1-HC 成对防火墙 / §2 人格规则 / §4 数据纪律 / §6.3 三网）仅占 5,261（16.6%）——
> **胖的从来不是约束，是叙述**。
>
> **零删除保证**：本次整理**未删改任何硬约束**。上述五处逐字保留、一字未动；§1.1 与 §1.1-HC
> 的成对防误删守卫亦原样在位（该守卫明定「删改任一须同步另一并经守密人裁定」，本次两者皆未触碰）。
> 移出正文的部分**全文在此**，可随时回填。
>
> **权威去向**：移出内容的溯源权威本就另有其档——决策历程 `memory/decisions.md` 与
> `memory/decisions-archive.md`、状态与进度 `memory/project-status.md`、逐文件知识
> OKF bundle（`kb_search` / `kb_get` 运行时导航）、命令细节各脚本自身与 `--help`。
>
> 追溯用；**运行时约束一律以 `CLAUDE.md` 正文为准**。

---

## 整理前全文（31,589 字符 / 547 行，逐字快照）

以下为整理前 `CLAUDE.md` 的完整原文，一字未改。正文现行版本见仓库根 `CLAUDE.md`；
两相对照即可看出本次移出了哪些叙述。
