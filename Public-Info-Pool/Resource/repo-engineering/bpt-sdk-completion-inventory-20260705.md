...
> 全部落地，工具面 15→20/24；版本升 0.7.0（后续 0.7.1 加 provider.cacheTtl）。`settingSources` 默认「不加载」
> 经守密人裁定为**有意保留**（KEPT-DIVERGENCE，非挂账）；pins.json 已钉 0.3.201（#478）。
...
- **`settingSources` 默认语义**（官方 live：省略=全加载 / 我方：省略=不加载）：**守密人裁定有意保留 load-nothing**
  （库形态直驱引擎不擅自读盘上 user/project/local 设置、要读显式 opt-in 更安全可预期）。**非挂账、非待升钉**——
  pins.json 早已钉 **0.3.201**（#478），且一致性套件**不测**此选项（内容盲仿真器无真实设置文件），故守 load-nothing
  **零差分影响、不报红**。定性为 **KEPT-DIVERGENCE**（有意保留的差异），非「唯一未做」。
  （注：`src/types.ts:1003` 旧注释仍写「待升钉」，属过时措辞，待下次 src 改动时顺带订正。）
- 六新 hook 的运行时**填充**（本轮仅 typed 加性，无自然钩点者诚实标 typed-not-fired）、其余 NEW-IN-DOCS
  typed-not-populated 字段的真填，属后续可选跟进。
