...
> **处置状态（2026-06-28 更新）**：守密人裁定**全量修复（含决策档案）**。本分支后续提交已落地
> **31/32** 条修复（断层 A 定位 11 条 + 装反护栏 / 断层 B 路径 6 条 / 断层 D 采集架构 / 断层 E wiki 数据 /
> 断层 F CI 政策 / 断层 G 退役壳 / 断层 H 死引用与人格 + C25 站点门户口径，另含 3 处同类定位残留补修）；
> 一致性脚本/单测已翻转、OKF bundle 已重生成、全量 `pytest tests/`（1962 passed / 7 skipped）全绿。
> **仅余 C6**（使命#3 主对接子项目 site vs game 的语义方向）属决策档案历史映射的真值分叉，
> **留守密人裁定后落地**——三选项见 §十二。
...
## 十三、唯一待裁项：C6 使命#3 主对接子项目（site vs game）

31/32 已落地（含 C25：site/CONTEXT L7 原误称「site = 使命#2 载体」已改为「对外门户/三轴发现入口」——使命#2 载体权威为 wiki，此项无歧义已修）。**仅 C6 的方向**属决策档案历史映射的真值分叉，须守密人裁定：

- **冲突**：CLAUDE.md §1.2 表写 使命#3 主对接 = **site / 全局**；但 v2.0 决策 M4（decisions.md L56）+ strategic-plan L34 + mission hub L26 写 **game（备扩展位）/ 全局**。
- **三选项**：
  - **(A，推荐) 全局为主，注明角色**：统一为「全局（game=团队练手备扩展位 / site=对外门户面）」，消除单选分叉，忠于 M4「game=训练场备扩展位」原文。改 CLAUDE.md §1.2 + strategic-plan L34 + mission hub L26 四处口径。
  - **(B) 以 CLAUDE.md「site / 全局」为准**：认 site 顶第三轴（game 已退主线不宜当载体）。改 strategic-plan L34、mission hub L26 为 site/全局。
  - **(C) 以决策 M4「game（备扩展位）/ 全局」为准**：回到 v2.0 原意。改 CLAUDE.md §1.2 为 game（备扩展位）/全局。
- **待守密人择一**，艾瑞卡即落地对应四处（或两处）同步。

> 小学生比喻：花名册第三栏「这活归谁」，总册写小 site、两本分册写小 game——得请管事的（守密人）拍板填哪个，三本才不打架；其余 31 处口径已全部对齐。
