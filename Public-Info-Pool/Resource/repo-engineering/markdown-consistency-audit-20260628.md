...
> **30/32** 条修复（断层 A 定位 11 条 + 装反护栏 / 断层 B 路径 6 条 / 断层 D 采集架构 / 断层 E wiki 数据 /
> 断层 F CI 政策 / 断层 G 退役壳 / 断层 H 死引用与人格，另含 3 处同类定位残留补修）；一致性脚本/单测
> 已翻转、OKF bundle 已重生成、全量 `pytest tests/`（1962 passed / 7 skipped）全绿。**余 C6 / C25**
> （使命#3 主对接子项目 site vs game 的语义方向）属决策档案历史映射的真值分叉，**留守密人裁定后落地**。
