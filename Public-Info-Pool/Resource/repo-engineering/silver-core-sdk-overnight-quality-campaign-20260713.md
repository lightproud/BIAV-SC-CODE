...
null usage 不覆写、空串 id 不采认、无参不发空 delta）。守密人续令「继续」后追加第六批 22 击杀（HTTP 状态→错误类型全表、modelMap 应用与
Claude 警告互斥、request-id 捕获、闲置停摆/硬顶的韧性错误字段契约（双臂探针核实：
turnReplaySafe 按块数定值、midStreamTruncation 专属硬顶/截断类）、退避期中止、URL 规整、
env 空值回退、parseRetryAfterMs 边角（'-1' 经 Date.parse 当过去日期→0 的怪癖入档）、
翻译器兜底），终局 **85.55%**（62.81→69.01→74.10→78.64→80.60→85.55，六轮；
击杀 829 / 存活 136 / 无覆盖 4）——**守密人核准的 85%+ 目标达成**。
残余 136 存活 = 已论证等价类（spread-undefined 经 JSON 序列化隐形、可选链在不可空路径上等）
+ 显式放弃类（日志文案/抖动算术/监听器微观——防过拟合，见「为何不追 100%」判断），
清单在 `reports/mutation/mutation.json`（gitignore，按需重生成）。
比喻：油田六轮开采到 85.5%，剩下的井要么地质上就是干的（等价），要么注水成本高过油价（过拟合）——账本写实收工。
