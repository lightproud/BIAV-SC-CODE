...
P0（对应需求档 R1.3 本地命令最小集）：**已全组闭环（2026-07-11）**——`/compact` OBS-012、
`/help` / `/resume` OBS-013/014（本环境未注册、fail-closed 幂等回执）、`/clear` OBS-015
收官（守密人触发 + 截图双视角）；带聚焦 args 的 `/compact <指令>` 变体未测，降级 P2；
P1：`/permissions` `/mcp` `/usage` `/hooks` `/memory`；
P2（展开型对照样本，验证四分类判据）：`/review`、`/compact <指令>` 变体（/loop 动态自调步
已于 OBS-011 闭环，2026-07-11）。
（/loop 固定模式与 Cron 生命周期已于 OBS-005 闭环。）**挂账 `memory/todo.md` #T12 已随
OBS-015 销案；P1 / P2 为随缘续记项，不另挂账。**
...
### OBS-015 `/clear`（2026-07-11，守密人会话内触发，T12 A 类 P0 组收官件；守密人截图 + 引擎座席双视角合璧）

- **可见物（引擎座席侧，清除后新窗第一手）**：新上下文首条用户回合 = CLAUDE.md
  system-reminder（照常注入）+ A 类三件套事后通报——`<local-command-caveat>`（勿响应告诫）+
  `<command-name>/clear</command-name>` + `<command-message>clear</command-message>` +
  `<local-command-stdout>` **空**（零字符；对照 /model「Set model to…」、/compact
  「Compacted 」——「成功但无话可说」的第三种 stdout 形态）。无 args 标签（未传参）。
  **零蒸馏残留**：无九节摘要、无文件状态重放、无 transcript 无损指针——与 /compact
  （OBS-012 五段重建）构成两极对照。注册表重广播照常发生（deferred 工具清单 / 代理类型 /
  MCP 服务端说明整批重投，与 OBS-012 第⑤段同构）。
- **可见物（守密人侧，移动端截图一手）**：会话时间线上 /clear 呈用户气泡 →
  「Initialized session」可展开回执（三步勾选：Set up a cloud container / Cloned 1
  repository / Started Claude Code）→「Context cleared」波浪分界线；**「Diff +36 -3」与
  「View PR」徽章跨清除存续**（+36 -3 正是本分支未合并的 OBS-013/014 两笔提交、PR #597
  绑定不断——会话级 Git / PR 状态不随上下文清除）。
- **状态不变量（时间线铁证）**：容器启动 10:24:45 UTC（uptime 945s @ 10:40 实测回推）、
  OBS-013/014 提交 10:34 / 10:36 出自本工作区（reflog 一手）、/clear 触发 ~10:37
  （截图 18:37 UTC+8）——**容器 uptime 跨越清除时刻**，工作区 / 分支 / 提交 / 上传件 /
  会话身份（UUID 路径不变）/ 指定分支任务头全数存续。清的是对话上下文，不是会话，
  更不是容器。（UI 层「Initialized session」回执呈现于 /clear 气泡与分界线之间，但铁证
  排除「清除触发容器重建」——判读为清除后时间线移除旧回合、原初始化回执重浮出；
  UI 排序语义单样本不可证，仅记形态。）
- **机制推断**：/clear = A 类纯本地 + **上下文归零原语**。窗口从零重建：系统提示 +
  CLAUDE.md + 注册表重广播，而新窗第一件「内容」就是清除命令自身的事后通报——引擎被
  明确告知「发生过清除」，不会失忆而不自知。与 /compact 的分工：一台是蒸馏机
  （摘要 + 重放 + 指针，保任务连续性），一台是橡皮擦（全弃，只留「擦过」的记号）。
- **BPT 映射**：需求档 R1.3（本地命令最小集 /clear）落为**上下文归零原语**规格五件：
  ① A 类三件套 wrapper、成功时 stdout 空；② 新窗首件内容 = 清除事后通报（防失忆不自知）；
  ③ 注册表重广播与 /compact 共用同一台重投机；④ harness 态（会话身份 / 工作区 / 调度 /
  Git-PR 绑定）一律不动——清对话不清会话；⑤ UI 侧持久徽章（diff / PR）跨清除存续，
  时间线插入显式分界标记。小学生比喻：/compact 是把旧笔记浓缩成一页大事记塞进新本子；
  /clear 是直接换一本空本子，第一页只写一行「旧本子已收走」——书包（工作区）、课桌
  （容器）、学籍（会话身份）都原封不动。
- **T12 销案注**：A 类 P0 组四件（/compact /help /resume /clear）+ /loop 动态自调步全数
  闭环，todo #T12 随本条销案；P1 / P2 尾巴留待观测清单随缘续记，不另挂账。
