...
>
> **裁定与落地状态（2026-07-26 同日更新）**：守密人对 F1–F4 四项交互裁定完毕（**均取推荐案**），
> 四条**已于 0.78.0 全部落地**（见 `projects/silver-core-maestro-sdk/CHANGELOG.md` 0.78.0）。
> 原一次性探针已**升格为常驻回归锁** `tests/design-review-20260726.test.ts`（27 例）+
> `tests/terminal-vocabulary.test.ts`（11 例，含治理守卫），测试 362 → **400**。
> **F5（「重开」语义）仍挂待裁**；F3 附带发现的 `Scheduler` 全表扫**不在裁定射程、未纳入本轮**（诚实记录）。
> 各条正文保留**审视当时**的原始状态描述不改写——审视档是当时的证据，不是变更后的说明书。
