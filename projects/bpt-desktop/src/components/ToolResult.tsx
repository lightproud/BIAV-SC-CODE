    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        <button
          ...
          className="w-full text-left rounded-lg border border-bpt-border bg-bpt-bg/50 p-3 hover:bg-bpt-border/20 transition-colors"
        >
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-xs font-medium ${isError ? 'text-red-400' : 'text-emerald-400'}`}>
              {isError ? '工具错误' : '工具结果'}
            </span>
            <span className="text-xs text-bpt-muted">{tool.toolName}</span>
            <svg
              className={`ml-auto w-3.5 h-3.5 text-bpt-muted transition-transform ${expanded ? 'rotate-180' : ''}`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
          ...
          {!expanded && (
            <pre className={`text-xs truncate ${isError ? 'text-red-300/70' : 'text-bpt-text/60'}`}>
              {preview}
            </pre>
          )}
          ...
          {expanded && (
            <pre className={`text-xs whitespace-pre-wrap break-words max-h-80 overflow-y-auto rounded p-2 mt-1 ${
              isError ? 'bg-red-500/10 text-red-300/80' : 'bg-bpt-border/30 text-bpt-text/80'
            }`}>
              {displayContent}
            </pre>
          )}
