...
const TYPE_COLORS: Record<Artifact['type'], string> = {
  code: 'bg-bpt-gold/20 text-bpt-gold',
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-bpt-border">
        <button
          onClick={handleCopy}
          className="px-2 py-0.5 text-xs rounded bg-bpt-border text-bpt-muted hover:text-bpt-gold transition-colors"
        >
          {copied ? '已复制 ✓' : '复制'}
        </button>
        <button
...
function VersionControls({
  ...
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-bpt-border">
      ...
      <button
        ...
        className="p-0.5 rounded hover:bg-bpt-border text-bpt-muted hover:text-bpt-text disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        title="上一版本"
      >
      ...
      <div className="relative">
        <button
          ...
          className="px-1.5 py-0.5 text-xs rounded hover:bg-bpt-border text-bpt-muted hover:text-bpt-text transition-colors"
        >
        ...
        {dropdownOpen && (
          <>
            ...
            <div className="absolute top-full left-0 mt-1 z-20 bg-bpt-surface border border-bpt-border rounded shadow-lg min-w-[160px] py-1">
              {versions.map((v, i) => (
                <button
                  key={i}
                  onClick={() => {
                  ...
                  className={`w-full text-left px-3 py-1 text-xs hover:bg-bpt-border transition-colors flex items-center gap-2 ${
                    i === currentVersion ? 'text-bpt-gold' : 'text-bpt-text'
                  }`}
                >
                  <span>v{i + 1}</span>
                  <span className="text-bpt-muted">{formatTimestamp(v.timestamp)}</span>
                  {i === versions.length - 1 && (
                    <span className="text-[10px] text-bpt-muted ml-auto">最新</span>
                  )}
                </button>
              ))}
            </div>
      ...
      {currentVersion > 0 && (
        <button
          onClick={onShowDiff}
          className="ml-1 px-1.5 py-0.5 text-[10px] rounded bg-bpt-border text-bpt-muted hover:text-bpt-gold transition-colors"
          title="与上一版本对比"
        >
          Diff
        </button>
...
export default function ArtifactsPanel({ artifacts, onClose }: Props) {
  ...
  return (
    <div
      className={`flex flex-col h-full bg-bpt-surface border-l transition-colors ${
        isNotLatest ? 'border-yellow-500/50' : 'border-bpt-border'
      }`}
      style={{ width: '40%', minWidth: 320 }}
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-4 h-12 shrink-0 border-b border-bpt-border">
        <div className="flex-1 min-w-0 flex items-center gap-2">
          <span className="text-sm font-medium text-bpt-text truncate">{displayArtifact.title}</span>
          ...
          {displayArtifact.versions.length > 1 && (
            <span className="px-1.5 py-0.5 text-[10px] rounded bg-bpt-border text-bpt-muted">
        ...
        <button
          onClick={onClose}
          className="p-1 rounded hover:bg-bpt-border text-bpt-muted hover:text-bpt-text transition-colors"
          title="关闭面板"
        >
      ...
      {artifacts.length > 1 && (
        <div className="flex gap-0 border-b border-bpt-border overflow-x-auto shrink-0">
          {artifacts.map((art, i) => (
            <button
              key={art.id}
              onClick={() => {
              ...
              className={`px-3 py-1.5 text-xs whitespace-nowrap border-b-2 transition-colors ${
                i === Math.min(activeIndex, artifacts.length - 1)
                  ? 'border-bpt-gold text-bpt-gold'
                  : 'border-transparent text-bpt-muted hover:text-bpt-text'
              }`}
            >
              {art.title}
              {art.versions.length > 1 && (
                <span className="ml-1 text-[10px] text-bpt-muted">v{art.versions.length}</span>
              )}
            </button>
          ))}
        </div>
