    ...
    <div className="relative">
      <button
        className={`px-2 py-1 text-xs rounded transition-colors ${
          systemPrompt
            ? 'bg-bpt-gold/20 text-bpt-gold-bright hover:bg-bpt-gold/30'
            : 'text-bpt-muted hover:bg-bpt-border hover:text-bpt-text'
        }`}
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 w-80 bg-bpt-bg-secondary border border-bpt-border rounded-lg shadow-xl p-3 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-bpt-text">系统提示</span>
            <select
              className="text-xs bg-bpt-bg border border-bpt-border rounded px-2 py-1 text-bpt-text"
          ...
          <textarea
            ref={textareaRef}
            className="w-full h-28 text-xs bg-bpt-bg border border-bpt-border rounded p-2 text-bpt-text resize-none focus:outline-none focus:ring-1 focus:ring-bpt-gold/50 placeholder:text-bpt-muted"
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              className="text-xs text-bpt-muted hover:text-bpt-text"
              onClick={() => {
                setLocalValue('')
