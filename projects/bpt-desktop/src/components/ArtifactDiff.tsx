  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-bpt-border">
        <span className="text-xs font-medium text-bpt-muted">差异对比</span>
        ...
        <button
          onClick={onClose}
          className="px-2 py-0.5 text-xs rounded bg-bpt-border text-bpt-muted hover:text-bpt-text transition-colors"
        >
          关闭
        </button>
      </div>
        ...
        {numberedLines.map((line, i) => {
          ...
          return (
            <div key={i} className={`flex ${bgClass}`}>
              <span className="w-10 shrink-0 text-right pr-1 text-bpt-muted/50 select-none border-r border-bpt-border/50">
                {line.oldNum ?? ''}
              </span>
              <span className="w-10 shrink-0 text-right pr-1 text-bpt-muted/50 select-none border-r border-bpt-border/50">
                {line.newNum ?? ''}
