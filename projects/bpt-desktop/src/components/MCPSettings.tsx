...
export default function MCPSettings({ onClose }: Props) {
  ...
  const refresh = useCallback(async () => {
    const [cfg, srvs] = await Promise.all([
      window.bpt.mcpGetConfig(),
      window.bpt.mcpListServers(),
    ])
    setConfig(cfg)
    setServers(srvs)
  }, [])
  ...
  useEffect(() => {
    if (expandedServer) {
      window.bpt.mcpListTools(expandedServer).then(setTools)
    } else {
      setTools([])
    }
  }, [expandedServer])
  ...
  async function handleStart(name: string) {
    ...
    await window.bpt.mcpStartServer(name)
    await refresh()
  ...
  async function handleStop(name: string) {
    setLoading((prev) => ({ ...prev, [name]: true }))
    await window.bpt.mcpStopServer(name)
    await refresh()
    setLoading((prev) => ({ ...prev, [name]: false }))
  }
  ...
  async function handleDelete(name: string) {
    ...
    await window.bpt.mcpSaveConfig(newConfig)
    await refresh()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-[600px] max-h-[80vh] bg-bpt-surface border border-bpt-border rounded-2xl shadow-2xl flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-bpt-border shrink-0">
          <h2 className="font-serif text-bpt-gold-bright text-lg">MCP 服务器</h2>
          ...
          {editingServer ? (
            ...
            <>
              {servers.length === 0 && Object.keys(config.mcpServers).length === 0 && (
                <div className="text-bpt-muted text-sm text-center py-8">
              ...
              {Object.keys(config.mcpServers).map((name) => {
                ...
                return (
                  <div key={name} className="border border-bpt-border rounded-xl overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-3">
                      ...
                      >
                        <span className="text-sm font-medium text-bpt-text">{name}</span>
                        <span className="text-xs text-bpt-muted ml-2">
                          {statusLabel(status)}
                    ...
                    {isExpanded && status === 'running' && (
                      <div className="border-t border-bpt-border px-4 py-3">
                        <div className="text-xs text-bpt-muted mb-2">可用工具:</div>
                        {tools.length === 0 ? (
                          <div className="text-xs text-bpt-muted/60">无工具</div>
                        ) : (
                          <div className="space-y-1.5">
                            {tools.map((tool) => (
                              <div key={tool.name} className="text-xs">
                                <span className="text-bpt-gold font-mono">{tool.name}</span>
                                {tool.description && (
                                  <span className="text-bpt-muted ml-2">{tool.description}</span>
                                )}
                              </div>
                            ))}
                          </div>
        ...
        {/* Footer */}
        {!editingServer && (
          <div className="flex justify-end px-6 py-4 border-t border-bpt-border shrink-0">
            <button
              onClick={handleAddNew}
              className="px-4 py-2 rounded-lg text-sm bg-bpt-gold/20 text-bpt-gold hover:bg-bpt-gold/30 transition-colors"
            >
              + 添加服务器
            </button>
          </div>
...
function ActionButton({
  ...
  const variants = {
    default: 'bg-bpt-gold/20 text-bpt-gold hover:bg-bpt-gold/30',
    danger: 'text-red-400 hover:bg-red-500/10',
    ghost: 'text-bpt-muted hover:text-bpt-text hover:bg-bpt-border',
  }
  return (
...
function ServerEditor({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-medium text-bpt-text">
        {editing.isNew ? '添加 MCP 服务器' : `编辑: ${editing.name}`}
      </h3>

      {editing.isNew && (
        <div>
          <label className="block text-xs text-bpt-muted mb-1.5">服务器名称</label>
          <input
            type="text"
            value={editing.name}
            onChange={(e) => onChange({ ...editing, name: e.target.value })}
            placeholder="my-mcp-server"
            className="w-full bg-bpt-bg border border-bpt-border rounded-lg px-3 py-2 text-sm text-bpt-text placeholder-bpt-muted/50 focus:outline-none focus:border-bpt-gold-dim transition-colors"
          />
        </div>
      )}

      <div>
        <label className="block text-xs text-bpt-muted mb-1.5">命令 (command)</label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-xs text-bpt-muted mb-1.5">参数 (每行一个)</label>
        <textarea
          value={argsText}
      ...
      <div>
        <label className="block text-xs text-bpt-muted mb-1.5">环境变量 (每行 KEY=VALUE)</label>
        <textarea
          value={envText}
