...
async function readFileAsAttachment(file: File): Promise<Attachment | null> {
  ...
  if (ext === '.pdf') {
    try {
      ...
      const result = await window.bpt.parsePdf(base64, file.name)
...
export default function DropZone({ children, onFilesDropped }: Props) {
  ...
  return (
    ...
    >
      {children}
      {isDragging && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-bpt-bg/80 border-2 border-dashed border-bpt-gold/50 rounded-lg pointer-events-none">
          <div className="text-bpt-gold text-lg font-medium">
            拖放文件到此处
          </div>
        </div>
      )}
