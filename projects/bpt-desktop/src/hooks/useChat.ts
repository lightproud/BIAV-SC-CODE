...
export function useChat() {
  ...
  useEffect(() => {
    const cleanup = window.bpt.onChatStream((_event, data) => {
      switch (data.type) {
        case 'meta':
  ...
  const approveToolUse = useCallback(async (toolUseId: string, approved: boolean, alwaysAllow?: boolean) => {
    ...
    await window.bpt.approveToolUse(toolUseId, approved, alwaysAllow)
  }, [])
  ...
  const stopStreaming = useCallback(async () => {
    await window.bpt.stopStreaming()
    setIsStreaming(false)
    setPendingTools([])
  }, [])

  const loadConversation = useCallback(async (id: string) => {
    const msgs = await window.bpt.getMessages(id)
    setMessages(msgs)
    ...
    try {
      const usage = await window.bpt.getSessionUsage(id)
  ...
  const editAndResend = useCallback(
    async (messageId: string, newContent: string, provider: string, model: string) => {
      ...
      if (conversationId) {
        await window.bpt.editMessage({ conversationId, messageId, content: newContent })
      }
  ...
  const regenerate = useCallback(
    async (provider: string, model: string) => {
      ...
      if (conversationId) {
        await window.bpt.regenerateMessage({ conversationId, afterMessageId: lastUserMsg.id })
      }
