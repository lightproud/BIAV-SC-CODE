...
注意：Discord CDN 链接约 24h 过期；当日实时跑可全量下载，历史日期多已 404（如实记录）。

用法：
  python projects/news/scripts/collect_fanart.py --date 2026-06-01 --out projects/news/data/fanart/2026-06-01
输出：out/ 下图片文件 + gallery_manifest.json（每条含 source/channel/author/text/file/status）。
"""
import os, json, re, argparse, hashlib, urllib.request, urllib.error

ROOT = "projects/news/data"
FANART_CH = ["同人创作", "art-and-memes", "官方素材", "official-materials",
             "晒卡分享", "art-and", "fanart", "创作", "绘"]
    ...
    for d, name in dir2name.items():
        ...
        for line in open(fp, encoding="utf-8"):
            ...
            for att in (m.get("attachments") or []):
                ...
                status = fetch(url, os.path.join(a.out, fn))
                gallery.append({"source": "discord", "channel": name,
                                "author": m.get("author_id", ""),
                                "text": (m.get("content") or "")[:200],
                                "orig_filename": att.get("filename", ""),
