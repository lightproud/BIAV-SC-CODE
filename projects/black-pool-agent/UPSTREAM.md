...
| pin tag | **`v2026.9.21`**（commit `d337b736aa1e8ebecfab043842d13e4a2d2f48a3`，2026-09-21，引擎版本 0.21.4） |
...
| 2026-09-07 | `v2026.8.31` | 0.21.0 | 周更例程移 pin。**首轮闭环 exit 4（换装后回归网红），经两处人工修复才绿**（原备注沿用引擎默认措辞「闭环全绿」，抹平了过程；2026-09-07 据该轮提交 `b4369209` 信息补录实况）：① 裸词换装误伤上游新增的保留字用例 `data.identity.test.ts`「renamed bots stay taggable」——把夹具里的 `mentionNameForms('Hermes')` 扫成了 `('Black Pool')`，而实现侧比对的保留字表锚在内部句柄 `hermes`（与显示品牌无关），断言遂对不上；按 find-in-page 先例加两条 BRAND_POST_RULES 把夹具与其说明注释还原为上游原字面。② 宪章哨兵随上游改词：TUI 提示由 “running in” 改 “in”；**WebUI 提示项整条删除**——上游 PR #97873 判定其为死码（无任何代码路径构造 `platform="webui"`）并在源码留下「勿复活此文案」注记。修复后闭环与三项守卫全绿 |
| 2026-09-19 | `v2026.9.14` | 0.21.3 | 周更例程自动移 pin（闭环绿；2 条已知环境缺口经受控闸门放行，逐条见 build/desktop-env-gaps.json） |
| 2026-09-23 | `v2026.9.21` | 0.21.4 | 守密人派发「拉取更新 0.21.4」（非周更例程档期；5,173 提交）。**首轮闭环 exit 3**（锚点点火台账拦下两条内网规则全树零命中），人工重锚后次轮绿：① `intranet#1` About 自更新区收尾——上游把 About 拆成「应用更新 / 卸载」两子页、Danger zone 改由 `includeUninstall` 门控，锚点随之改；**同组开头规则 #0 仍命中**，若无台账拦截即产出只有开括号的 JSX、桌面端构建失败。② Billing 入口——上游加 overview/plans 子项并缩进两级，改为开头两行 + 收尾两条分锚，不再抄子项内容。③ **补封新入口**：上游新增独立「卸载」子页（设置导航与搜索均可直达），按 2026-08-02「便携包无安装器」裁定封死路由分支并摘除子页登记。宪章哨兵随改一条、新增两条。**构建腿首用**（`npm run build` 46 秒通过）；vitest 11,109 过 / 2 红（在册经受控闸门放行）/ 15 跳过 |

## 同步例程（移 pin 时照此执行）

**自动生产已移交黑池（守密人 2026-09-25 裁定）**：银芯侧不再有周更例程自动移 pin，以下为手办时的地面真相。原文如下（保留作历史）——周更例程每周一 00:00（北京，UTC 周日 16:00）自动跑完下述四步，
