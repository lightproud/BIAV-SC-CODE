...
- **2026-09-21 · 第二处判定链缺口：验证网止于 vitest，不跑 build / typecheck（待守密人裁定）**：
  ...
  **缺口本身仍开着**：会话侧闭环与组装线 regression-net **都只跑 vitest**，构建期的模块解析
  ...
  属工程变更，呈守密人裁定后再动。
- **2026-09-21 · 上两条销案（守密人两项裁定）**：① **构建盲区已补**——`npm run build` 纳入换装后
  回归网（`verify.run_desktop_build`，闭环与 CI 入口共用；本地实测约 36 秒）。构建腿**先于
  vitest 跑**（塌了不必再等十几分钟），且**不可豁免**：环境缺口台账只管 vitest 的用例级失败，
  构建塌了说明这棵树根本装不出来，没有「在册」一说。守卫两例（构建塌 → 整条未过，vitest 绿
  不绿都一样 · 两条腿都必须真调 `npm run build`）。② **`scripts/desktop-update` 裸词铺开评估
  销案**——维持五处字面量的定点换装规则：自更新窗口的用户可见文案已被定点规则盖住、上游新
  用例也盯着；同目录其余是 `windows.ps1` / `posix.sh` 等脚本，内含大量 `HermesUpdateJob` /
  `Invoke-HermesStep` / `$HermesHome` 复合标识符与 `/opt/Hermes/hermes` 一类夹具路径，铺裸词
  的风险大于收益。此条不再列为开着的缺口。
