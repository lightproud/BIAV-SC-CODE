...
- **2026-08-04 · 定位（台架实测，可复现）· 秒级放大器 = 端点「被丢包」而非「被拒绝」**：
  银芯容器内把打好全部补丁的树装成可运行网关，用本轮埋点自测，得到硬数据——
  **同一份代码、同一台机器、只改 provider 端点是否回包**：`model.options` 首次
  **4058–6267 ms（丢包）vs 232 ms（立刻 RST）**，首轮交互突发 **9.7 s vs 1.05 s**，
  热态（目录已缓存）全串 ~0.83 s、单条 ~30 ms。**机制**：应用首次触碰某外部端点
  （模型选择器 / 目录 / 定价 / 账户探测）要等满超时；防火墙 DROP 不回 RST 时，
  这笔账就是秒级，量级与现场报的「5-10 秒」吻合；缓存落 `home/` 后不再探测，
  故症状是「用着用着某个操作忽然卡一下」，不是稳定慢。**另两条实测结论**：
  ① 网关稳态 RPC 是毫秒级（跨 30 秒 check_fn TTL 复测四轮无回涨），
  **持续的每交互 5-10 秒不可能来自稳态网关工作**，只能来自冷窗反复重演（进程/连接被回收）
  或网关之外；② 冷窗里内联 lane 会被并发的池内重活挤成秒级（实测 wake.status 2.8 s，
  正常 50 ms）——即上游 #60800 家族现象，按守密人裁定不追。
  **诚实记录一次失败尝试**：曾在 `_urlopen_model_catalog_request` 加超时上限旋钮，
  严格 A/B（全新 HERMES_HOME、只改这一变量）实测 **4058 vs 4063 ms 无差别**——
  没打中真正的调用路径，**故未采纳、未入库**（上游 picker 探针本身已封顶 1.5 s，
  余下秒数散在其它各自有界的探针里，属上游设计账）。落地的是**现场可执行的处置**：
  分诊器第 9 节逐端点报 OK / REFUSED / DROPPED 并点名放大器，RUNBOOK 给三级处置
  （防火墙改回 REJECT / 配对代理 / 首部署放行一次喂热缓存）。
