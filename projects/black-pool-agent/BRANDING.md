...
| **状态栏版本芯片（守密人 2026-08-03「右下角版本号是 0.19.1」）** | ①公版：芯片显示黑池版本 `0.1.0`（与 About 同口径，unknown 态语义保留）②私有版：client / backend 两枚芯片点击不再打开更新覆盖层（自更新入口第六、七处收口） | 3 处 |
...
| **默认语言简体中文（守密人 2026-08-03 裁定；同日实机复盘补真源头）**：desktop 启动从后端取 `display.language`，其显式默认 `en` 压过前端缺省——两级同改（后端 config 默认 + 前端 `DEFAULT_LOCALE` 兜底） | 基座层后置规则：desktop 缺省 locale 单一真相源 `DEFAULT_LOCALE` `'en'`→`'zh'`（无系统语言探测，配置未设时生效；已设用户不受影响）；公私两版同得（语言偏好属产品定制非内网适配）；哨兵入 `test_brand_patch_sentinels` | 1 处 |
