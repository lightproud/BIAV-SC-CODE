...
> bpa-dev\deploy」；2026-08-03 终编：仓库侧本目录即部署件全集，生产工具另居 build/）：车间四脚本
> （组装 / 验证 / 部署 / 回滚）+ 补丁应用器 + 生命周期探测器（`bpa_lifecycle.py`）+ 启动器家族
> （launcher / 监督器 / venv 自愈）+ SOUL 模板 + 整包更新器 + CLI 别名，一目录拿全。整目录拷入
...
2. **验证（可选）** `verify.cmd`：只对 `staging\BlackPool` **就地体检**（结构 → venv 自愈 →
   运行时导入自检），零部署位触碰——**普通验证不影响正在使用的 BPA**（守密人 2026-08-03
   部署体验裁定「验证与部署分离」）。桌面端真机冒烟仍需部署后进行（单实例约束，
   staging 直启会转交给在跑实例反而误判）。
3. **部署** `deploy.cmd [部署目录] [/y]`：目标取参数 > 环境变量 BPA_DIR > `config\deploy-target.txt`
   （文件里写一行地址，**支持相对路径、按车间根 bpa-dev\ 解析**——写 `..\black-pool-agent`
   则整棵树搬盘符零改配置；绝对路径如 `E:\BIAV-BP\black-pool-agent` 亦可。写好后**双击即部署**，rollback 同理）。
   **部署体验（守密人 2026-08-03 裁定五项）**：
   - 先经 `bpa_lifecycle.py status` 探测部署位进程——检出在跑即**明确提示「部署将重启 BPA」**并要确认；
   - 检测到**疑似任务执行中**（CPU 采样 + `home\logs` 近 60 秒写入双信号，启发式）时给三选：
     等任务空闲后自动继续（每 10 秒复测，约 10 分钟复问一次）/ 立即重启 / 取消；
   - 确认后**先优雅退出**（`bpa_lifecycle.py graceful` 投 WM_CLOSE、守窗 20 秒，会话状态正常落盘），
     超时才 `kill_by_path.py` 强杀残余——强杀从默认动作降级为最后手段；
   - 关停后才做 `home\` 保全（文件静止态拷贝）→ 旧版让位 `<目录>.old` 回滚位 → 成品上位；
   - **部署前在跑则完成后自动重启** Black Pool（恢复会话窗口）；此前没在跑则只给启动提示。
   - 无人值守 / 脚本链调：加 `/y` 跳过全部确认（busy 时同样直接继续）。
4. **回滚** `rollback.cmd <部署目录>`：一键回切 `.old`，问题版留 `.failed-*` 供取证。
5. **启动**：deploy 会在部署位生成带图标的 **`Black Pool.lnk`**（图标取自主程序，可拷到桌面/任务栏）；
