...
INTRANET_POST_RULES = [
    ...
    # 私有 provider 注册表晚同步（守密人 2026-08-04 内网补丁通用化收编，乙案）：
    # 上游在 import 期一次性把 plugins/model-providers/ 的 api-key 供应商并入
    # PROVIDER_REGISTRY；打包桌面端可能在私有插件（bpa-dev 注入）被发现之前就
    # import 了 auth——改为函数化 + resolve_provider() 边界再同步一次。
    # 通篇零内网值；锚点唯一性已全树 grep 验证（lesson #58 纪律）。
    (
