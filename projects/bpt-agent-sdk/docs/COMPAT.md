...
|---|---|---|---|
| **Built-in tool descriptions language (i18n-zh)** | KEPT-DIVERGENCE | English tool descriptions on the wire | **TRANSLATED TO CHINESE in-place and shipped on the wire** (keeper ruling B, 2026-07-08 — in-place over a selectable variant). A deliberate break from the faithful-English-reproduction axis and its provenance guard. Tool names + wire parameter names stay English (identifiers); only prose is translated. Rolled out in batches: **v0.19.0 = Read/Edit/Write/Grep/Glob; v0.20.0 = Task*/TodoWrite/WebFetch/WebSearch/AskUserQuestion/ExitPlanMode/EnterWorktree; v0.21.0 = Monitor; v0.22.0 = Workflow; v0.23.0 = Bash (+ git protocol + 18 sandbox fragments — safety-critical, git-safety and dangerouslyDisableSandbox logic preserved exactly)**. **COMPLETE — every built-in tool description is now Chinese.** All tools dropped out of `TOOL_DESCRIPTION_PROVENANCE` (now empty); covered by `tests/tool-descriptions-i18n-zh.test.ts`. **Scope boundary (still English):** runtime execution feedback — the sandbox-failure stderr hint (`src/sandbox/evidence.ts`) and the win32 cmd-habit hint / mandatory-mode refusal (`src/tools/bash.ts`) — is a distinct failure-time surface, not a tool description, and is intentionally left English (a possible follow-up). A/B quality comparison + a decisions.md entry remain follow-ups (keeper-gated) |
| **System prompt language (i18n-zh)** | KEPT-DIVERGENCE | English main-loop system prompt on the wire | **being TRANSLATED TO CHINESE in-place** (keeper ruling B, 全部推进 including main-loop). **Phase 2 batch A (v0.24.0) = the v5 default main-loop system prompt** — all 30 behavioral-contract fragments (security-assistance, doing-tasks, tool-use, executing-actions-with-care, outcome-first communication, code style, emoji-avoidance, safety) + the `可用工具：` label. Tool names + wire parameter names + command/code tokens (`git status`, `--no-verify`, `file_path:line_number`, `A -> B -> fails`, `"Sources:"`, CLAUDE.md) stay English; only prose is translated. Tool-gating / red-line invariants unchanged. Fragments flip `faithful:false` (English corpus-sync guard goes inert for CJK); the byte-identity golden fixture is regenerated; covered by `tests/prompt-fragments-i18n-zh.test.ts`. **Phase 2 batch B (v0.25.0)** = the judge/verdict/delegate generator prompts — hook-condition evaluators, the adversarial verifier prompt, and subagent framing (`GENERAL_PURPOSE_PROMPT` / `WORKER_FORK_FRAMING`); output contracts (JSON keys, `CONFIRMED`/`PLAUSIBLE`/`REFUTED` enum, tool tokens) kept English, parsers unchanged; `VERIFY_KEEP_RULE` (doc anchor, not on wire) stays English; covered by `tests/aux-prompts-i18n-zh.test.ts`. **Phase 2 batch C (v0.26.0)** = the 5 small generators (session-title / title+branch / session-name / away-summary / memory-files) + the 2 context-tip prompts; JSON keys/enums/placeholders (`{description}`/`{situations}`), the `claude/` prefix, `[user]`/`[project]` markers, and the few-shot examples kept English; covered by `tests/gen-tips-i18n-zh.test.ts`. **Phase 2 batch D (v0.27.0)** = the two big classifiers — `COMMAND_PREFIX_SYSTEM` (command-injection detection; `<policy_spec>` example block + `none`/`command_injection_detected` outputs kept English) and `BACKGROUND_STATE_SYSTEM` (state/tempo enums + JSON template + all few-shot example tails kept English); all 7 `GENERATOR_PROVENANCE` entries now translated; covered by `tests/classifiers-i18n-zh.test.ts`. **Phase 2 batch E (v0.28.0) = COMPLETE** — the opt-in harness variants `minimalStable` + v1–v4 translated (tool names + `file_path:line_number` + the calc.mjs example kept English; `可用工具：` replaces `Available tools:`). **Every tool description and system/generator/classifier prompt shipped on the wire is now Chinese.** **DELIBERATELY still English (documented scope boundary):** the conformance-LOCKED runtime `<env>` block (`environmentBlock` / `volatileTail` path-guidance — byte-asserted by conformance-l2-locks / api-surface / engine / cache-control; translating breaks official-parity) and the CLAUDE.md/AGENTS.md `<system-reminder>` wrapper, plus the runtime execution-feedback hints (sandbox-failure stderr, win32 cmd-habit hint) — all distinct from "descriptions and prompts". A/B quality comparison + a decisions.md entry remain follow-ups (keeper-gated) |
