...
|---|---|---|---|
| **Built-in tool descriptions language (i18n-zh)** | KEPT-DIVERGENCE | English tool descriptions on the wire | **being TRANSLATED TO CHINESE in-place and shipped on the wire** (keeper ruling B, 2026-07-08 — in-place over a selectable variant). A deliberate break from the faithful-English-reproduction axis and its provenance guard. Tool names + wire parameter names stay English (identifiers); only prose is translated. Rolling out in batches: **v0.19.0 = Read/Edit/Write/Grep/Glob; v0.20.0 = Task*/TodoWrite/WebFetch/WebSearch/AskUserQuestion/ExitPlanMode/EnterWorktree; v0.21.0 = Monitor; v0.22.0 = Workflow**; still English = **Bash only** (+ its git protocol + sandbox fragments — held for a later, safety-critical batch). Translated tools drop out of `TOOL_DESCRIPTION_PROVENANCE`; covered by `tests/tool-descriptions-i18n-zh.test.ts`. A/B quality comparison + a decisions.md entry are follow-ups (keeper-gated) |
