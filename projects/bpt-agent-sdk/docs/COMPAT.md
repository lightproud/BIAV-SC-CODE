...
| `result` success / error_max_turns / error_during_execution / `error_max_budget_usd` / `error_max_structured_output_retries` | PARTIAL | success arm carries ttft_ms + structured_output + deferred_tool_use + v0.3 `metrics`; error arm carries both `errorMessage: string` and the official-parallel `errors: string[]` (v0.4; always `[errorMessage]`). Fault-path subtype selection diverges from official 2.1.201: on a terminal non-retryable 400 and on mid-stream SSE truncation ours ends the session with a clean `result/error_during_execution`, where official surfaces the error as assistant text plus `result/success` then throws from the iterator (KD-L4-01 / KD-L4-02; conformance run-l4 l4-http400-non-retryable, l4-script-exhausted-400-terminal, l4-sse-truncated-text-turn, 2026-07-05); on a truncated TOOL turn official additionally executes the already-complete tool_use blocks, re-requests and completes `success` where ours abandons the turn (KD-L4-04; run-l4 l4-sse-truncated-tool-turn / l4-sse-truncated-tool-incomplete) - suspected our-engine graceful-degradation gap, tracked as deliberately-red engine findings in the M3 ratchet baseline |
