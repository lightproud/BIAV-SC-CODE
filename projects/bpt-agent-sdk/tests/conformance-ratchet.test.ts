...
describe('runnerKeyOf', () => {
  it('derives l1/l2/l3 keys and stays extensible to l4', () => {
    expect(runnerKeyOf({ generated_for: 'bpt-agent-sdk conformance L1 (stream grammar)' })).toBe('l1');
    expect(runnerKeyOf({ generated_for: 'bpt-agent-sdk conformance L2 (options semantics)' })).toBe('l2');
    expect(runnerKeyOf({ generated_for: 'bpt-agent-sdk conformance L3 (tool behavior differential)' })).toBe('l3');
    expect(runnerKeyOf({ generated_for: 'bpt-agent-sdk conformance L4 (fault injection)' })).toBe('l4');
  });
...
describe('buildRunners + serializeBaseline', () => {
  ...
  const l3 = {
    generated_for: 'bpt-agent-sdk conformance L3 (tool behavior differential)',
