...
## 情境八：阿里内部 idealab 网关（BIAV 生产场景）

> 档案编号：BPT-NEXT-IDEALAB
> 确认日期：2026-04-14 by 艾瑞卡（守密人提供档案）
> 适用场景：BIAV Studio 内部研发环境

idealab 是阿里内部统一 LLM 网关，**完整支持 Anthropic Messages API 协议**。bpt-next 接入时零能力损失——prompt caching / tool_use / 流式 content_block_delta 等 Claude 原生特性全部保留。

### 端点选择（严格锁定）

| 入口 | 用途 | BPT-NEXT 是否可用 |
|------|------|------------------|
| `https://idealab.alibaba-inc.com/api/anthropic/v1/messages` | API key 直调（`x-api-key` 头） | **唯一可用** |
| `https://idealab.alibaba-inc.com/code/v1/messages` | 浏览器 SSO 鉴权（Web IDE 集成场景） | **不可用**——API key 直调会失败 |

### 配置

```bash
export ANTHROPIC_BASE_URL="https://idealab.alibaba-inc.com/api/anthropic"
export ANTHROPIC_API_KEY="<idealab ak>"

cd projects/bpt-next/rust
./target/debug/claw doctor                        # 健康检查
./target/debug/claw prompt "say hello"            # 默认 sonnet
./target/debug/claw --model opus prompt "..."     # 高性能任务
./target/debug/claw --model haiku prompt "..."    # 低成本任务（需 .claw/settings.json 别名覆盖）
```
...
| 入参模型名 | 定位 | `claw` 别名适配 |
|-----------|------|----------------|
| `claude-sonnet-4-6` | 默认，通用 | `sonnet`（claw 内置别名直接匹配） |
| `claude-opus-4-6` | 高性能 | `opus`（claw 内置别名直接匹配） |
| `claude-haiku-4_5` | 低成本 | **需自定义别名**——claw 内置 `haiku` 展开为 `claude-haiku-4-5-20251213`，idealab 识别不了 |
| `qwen3-coder-plus` | Qwen 代码模型（网关包装为 Anthropic 格式） | 直接传完整名 |

### 命名陷阱警告（严格执行）

**idealab 三个 Claude 模型命名不一致**：

- Sonnet / Opus 用**连字符**：`claude-sonnet-4-6` / `claude-opus-4-6`
- Haiku 单独用**下划线**：`claude-haiku-4_5`（注意 `4_5` 的下划线）

本目录 `.claw/settings.json` 已预置 `haiku` 别名覆盖，守密人直接用 `--model haiku` 即可。若手动配置请勿误写为 `claude-haiku-4-5`。

### 凭据存放

- **项目级配置**（入仓，无凭据）：`projects/bpt-next/.claw/settings.json`（别名表）
- **守密人凭据**（gitignore，含 ak）：`projects/bpt-next/.claw/settings.local.json` 或用户级 `~/.claw.json`

`.claw/settings.local.json` 已在 `projects/bpt-next/.gitignore:9` 排除，**绝不会**入仓。

### 验证序列

```bash
cd projects/bpt-next/rust
./target/debug/claw prompt "reply ok-sonnet"                      # 默认 sonnet
./target/debug/claw --model opus prompt "reply ok-opus"
./target/debug/claw --model haiku prompt "reply ok-haiku"         # 别名覆盖生效
./target/debug/claw --model "claude-haiku-4_5" prompt "reply ok"  # 显式完整名
```

### 与其他情境的关系

- 本情境**取代**情境七（Anthropic 兼容本地代理）——idealab 是 BIAV 生产级网关，非本地调试
- 本情境**优于**情境三/四（OpenAI-compatible）——idealab 是 Anthropic-compatible 网关，保留 prompt caching 等 Claude 原生能力
- BIAV 内部 bpt-next 用户应**默认使用本情境**，无需再评估其他 provider
