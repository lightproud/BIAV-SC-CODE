 ...
 *   node scripts/check-mutation-ratchet.mjs --name ledger-state \
...
/**
 * NOTE: this is a deliberate twin of the agent SDK's identical guard, NOT an
 * oversight. The dependency-direction contract (SCS-REQ orchestrator-sdk §2)
 * forbids this package from reaching into ../silver-core-sdk by relative path,
 * scripts included, so the two copies are the only lawful arrangement. Keeper
 * ruling 2026-07-27: keep both copies, keep them functionally equal — this one
 * had fallen behind by the W3-1/W3-2 scoping+testability fix and is now level.
 *
 * Turn a Stryker `mutate` glob (e.g. `src/ledger/state.ts`,
 * `src/workflow/**​/*.ts`) into an anchored RegExp against a POSIX-style path.
