...
describe('emulator end-to-end (real stack, scripted model)', () => {
  it('runs a multi-tool agent loop with real fs + MCP side effects', async () => {
    await startServer((toolTurns, model, res) => {
      switch (toolTurns) {
        case 0: return streamToolUse(res, model, 'tu_1', 'Write', { file_path: 'notes.txt', content: 'hello from silver-core-sdk\n' });
    ...
    expect(fs.readFileSync(notes, 'utf8')).toBe('hello from silver-core-sdk\n');
