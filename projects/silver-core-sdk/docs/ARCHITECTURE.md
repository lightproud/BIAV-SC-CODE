# Silver Core SDK — Architecture & Module Contracts
...
- Headers: `anthropic-version: provider.apiVersion ?? '2023-06-01'`,
  `content-type: application/json`, `user-agent: silver-core-sdk/0.1.0`, plus
...
- `export class StdioMcpConnection` — spawn `command args` with merged env
  ...
  `{ name: 'silver-core-sdk', version: '0.1.0' }`) → store serverInfo →
...
(`@anthropic-ai/claude-agent-sdk` → `silver-core-sdk`), env vars, compat
