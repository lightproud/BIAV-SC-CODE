...
| 9 | Memory vs Claude Code auto-memory non-goals | **PARTIAL（扩大收编）**: adopted now — official markdown-link index entry format `- [<title>](<file path>) — one-line hook` (all three prompt sites) and the official two-state capacity warning (approaching tier at 80% + target size in both tiers). Remaining three (frontmatter type taxonomy, selective attachment, dream signal sources) go to a dedicated design round: todo T75, evaluation draft `Public-Info-Pool/Resource/proposal/memory-charter-expansion-eval-20260729.md` **T75 grilled 2026-07-29**: dream signal source ruled and SHIPPED (opt-in `transcripts` pointer block in buildConsolidationPrompt + the `consolidationToolOptions()` read-only harness floor — closing the old prompt-only "memory tool only" gap); frontmatter + selective attachment ruled straight to a design round — r1 requirements doc `Public-Info-Pool/Resource/repo-engineering/scs-req-memory-frontmatter-attachment-r1-20260729.md` **FINALIZED 2026-07-29 (fourth pass)**: §2.2 attachment shape as drafted (opt-in `memory.attachment`, hard ConfigurationError dependency on `schema:'frontmatter'`, 25600-byte cap, graceful degrade); pinned = always-attach bypassing the picker, not counted against maxFiles, budget-priority; when_to_save/body_structure prompts ruled **full official text injection** (verbatim + provenance, overruling the condensed-version recommendation). **Cards-relationship ruled same day (third pass): cards RETIRES** — keeper verified cards was never a Claude memory mode (R9, black-pool adaptation axis, honest BPT-EXTENSION) and ruled the memory surface converges on the Claude shape; removal folds into the frontmatter implementation step as ONE breaking migration (2.0.0, lockstep; testbed dream cards migrate with it, black-pool consumption to be confirmed verbally pre-implementation). Evaluation draft superseded by the r1 doc. |
