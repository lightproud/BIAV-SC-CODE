...
上游核对记录 0.112.3 无欠账 / 契约套件并发节:单命令原子性 + last-write-wins 两可执行
检查、不加版本令牌 / `assessMemoryStoreHealth` 深扫健康面:目录水位软 48 预警、腐烂度
mtime 不可得则标注、容量余量、supersede 链完整性、读写比——黑池做梦触发面);
③ `provider.capabilities` 端点能力声明(usage/promptCaching/thinking/parallelToolCalls,
线缆边界按声明降级、逐条 debug 报告、画像机制维持不立项)+ `options.continuationPrompt`
续跑片段(协议门控默认:openai-chat 开 / anthropic 关,未武装字节不变,双协议假端点
e2e 验证注入)。**3017 单测全绿 + 2 skipped**(+19)。
...
③ 迁移文档 `docs/MIGRATION-0.3x-to-0.52.md`（**已被 `docs/MIGRATION-0.3x-to-0.68.md` 取代**，2026-07-18 刷新为 0.3x→0.68 完整通道：斜杠退役 0.63.0 / MultiEdit 生命弧 / 锁步制 0.68.0 / npm 两连改名 / 黑池升级检查单；原三节：更名 0.41.0、
