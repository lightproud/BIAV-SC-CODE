...
「默认动末位」是**作者纪律**；守卫只锁两件可机检的——

① **步型单步**，查的是 **CHANGELOG.md 自身的版本序列**（相邻两条必须恰一位 +1、其下归零），
**不是 diff**：本仓 squash 合并长分支，一个合并提交合法地跨多次发版（分支切了
1.5.0/1.6.0/1.7.0/1.8.0，落到停在 1.4.1 的 main 上就是一条 1.4.1→1.8.0 的 diff），
按 diff 判单步会**把每次这样的合并误红**。台账才是免疫 squash 的地方——它列出所有存在过的版本，
所以「洞」正是该抓的缺陷（手改把 1.8.1 敲成 1.8.3，1.8.1/1.8.2 就永远没有条目，
而跳掉的号事后无法回收：家族锁步且消费方按 tarball 版本 pin）。0.35–0.37 三对历史异常
**显式冻结成棘轮**（已发布、已被 pin，不能重编号；但集合封闭，新违规藏不进去）。

② **首位移动**须在提交信息带 `Version-Bump-Override: <守密人裁定>` 尾行
（**诚实机制、非安全边界**——写提交的人就是写尾行的人；它的作用是让首位不能作为日常工作的
副作用悄悄移动，而必须留下一行可审计的历史）。
