...
- Pure research/exploration tasks (no implementation to plan)
...
export const WORKFLOW_DESCRIPTION = `Execute a workflow script that orchestrates multiple subagents deterministically. Workflows run in the background — this tool returns immediately with a task ID. Read the workflow's progress and its final result (the script's return value, JSON-serialized, plus the phase()/log()/per-agent transcript) with TaskOutput, and stop a run with TaskStop. The launch result also carries the runId for resume and the persisted script path. A script that fails its syntax check does NOT launch: that error comes back from this call directly.
