...
| hook | 触发 | 脚本 | 软失败规范 |
|---|---|---|---|
| SessionStart | 会话启动 | `.claude/hooks/session-start-sync.sh` | 失败时不阻塞会话，仅日志告警 |
| SessionEnd | 会话结束 | `scripts/session-end-distill.sh → session_distiller.py` | 失败时 digest 缺失，下次启动时无影响 |
| stop-hook（git-check）| Stop 事件 | `~/.claude/stop-hook-git-check.sh` | 检测未追踪文件，仅提示 |
| UserPromptSubmit | 用户输入 | `scripts/session_inject.py`（如已配） | 注入历史档案上下文 |
