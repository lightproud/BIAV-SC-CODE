...
清空已完工存量，历史版本 git 可追），`projects/news/scripts/` 为采集器层——其中
...
- **CI 硬门禁已生效（守密人 2026-07-11 完成 Ruleset 勾选，销 todo T19）**：main Ruleset 恢复
  ...
  up to date）。合并纪律：squash 合并前**等 PR 上 required `test` 检查跑绿**；改了代码 / 测试 /
  数据的 PR 仍应本地自跑 `pytest tests/` 先行兜底，**绝不**把「CI 会拦」当成「免自查」。
  历程：2026-06-21 撤检查改自查自合 → 2026-07-10 批准重启（撤检查的两条理由已随 07-02
  sparse checkout 改造消失）→ 2026-07-11 勾选生效。
